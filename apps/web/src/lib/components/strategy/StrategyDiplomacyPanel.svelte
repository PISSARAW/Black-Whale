...
<section class="mb-8 border-t border-sky-900/30 pt-6">
  <div class="mb-4 flex items-center justify-between">
    ...
    <span class="rounded bg-sky-900/40 px-2 py-1 text-[9px] font-bold uppercase tracking-widest text-sky-300">{pending.length} action{pending.length !== 1 ? 's' : ''}</span>
  </div>
  ...
  <label class="mt-4 block">
    <span class="mb-1 block text-[10px] font-bold uppercase tracking-widest text-sky-500/60">Interlocuteur</span>
    <select bind:value={selectedFactionId} class="w-full rounded-lg border border-sky-900/50 bg-[#060b14] p-3 text-xs text-sky-50 outline-none transition-colors focus:border-sky-400 focus:ring-1 focus:ring-sky-400">
  ...
  <label class="mt-4 block">
    <span class="mb-1 block text-[10px] font-bold uppercase tracking-widest text-sky-500/60">Proposition</span>
    <select bind:value={selectedAction} class="w-full rounded-lg border border-sky-900/50 bg-[#060b14] p-3 text-xs text-sky-50 outline-none transition-colors focus:border-sky-400 focus:ring-1 focus:ring-sky-400">
  ...
  <button class="mt-5 w-full rounded-lg border border-sky-900/60 bg-[#101827] px-4 py-2.5 text-xs font-bold uppercase tracking-widest text-sky-200 transition-colors enabled:hover:border-sky-500/50 enabled:hover:bg-sky-900/40 disabled:cursor-not-allowed disabled:opacity-40" type="button" disabled={!selectedFactionId} onclick={onqueue}
    >Formuler Proposition</button
  >
  {#if pending.length}
    <ul class="mt-4 flex flex-col gap-1.5 pl-4 text-[10px] font-medium text-sky-200/50">
      {#each pending as order (order.factionId)}
        <li class="relative before:absolute before:-left-3 before:top-1/2 before:h-1 before:w-1 before:-translate-y-1/2 before:rounded-full before:bg-amber-400">
