...
async function upsertOccupancy({ id, bodyId, consciousnessId, fromEventId, occupancyType = 'TRANSFERRED', certainty = 'CONFIRMED' }) {
...
async function upsertPresence({ id, bodyId, locationId, fromEventId, untilEventId = null, precision, certainty }) {
	await prisma.presence.upsert({
		where: { id },
		update: {
			entityType: 'BODY', entityId: bodyId, locationId, fromEventId, untilEventId, precision, certainty
		},
		create: {
			id, entityType: 'BODY', entityId: bodyId, locationId, fromEventId, untilEventId, precision, certainty
		}
	});
}
...
async function main() {
	...
	const kachoPresences = await prisma.presence.findMany({ where: { entityId: kacho.originalBody.id } });
	for (const presence of kachoPresences) {
		await prisma.presence.update({ where: { id: presence.id }, data: { untilEventId: kachoDeath.id } });
	}
