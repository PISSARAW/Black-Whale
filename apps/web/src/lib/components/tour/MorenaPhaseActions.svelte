  ...
  <div class="mt-4 rounded border border-[#d94f68]/60 bg-[#d94f68]/10 p-3">
    ...
    <div class="mt-2 flex flex-wrap items-end gap-2">
      {#each game.graveyard as card (card)}
        <button class="rounded border border-[#d94f68] p-1.5 hover:bg-[#d94f68]/30"
          onclick={() => (game = takeTheDeal(game, card))} title="{copy.deal.take} · {nameOfCard(card)}">
          <MorenaCard face={card} label={nameOfCard(card)} ink={cssInk(CARD_COLOURS[card])} />
        </button>
      {/each}
      <button class="rounded border border-[#444] px-3 py-1.5 text-xs text-[#FFFFF0] hover:border-[#FFD700]"
...
{:else if game.phase === 'asking'}
  ...
  <ul class="mt-2 space-y-1.5">
    {#each game.questions as question (question)}
      <li><button class="flex w-full items-center gap-3 rounded border border-[#333] p-2 text-left text-sm text-[#FFFFF0]/85 hover:border-[#d94f68] hover:text-[#FFFFF0]" onclick={() => ask(question)}>
        <MorenaCard face={question} label={questionCopy(question).short} ink={cssInk(DEALER_COLOUR)} />
...
{:else if game.phase === 'settling'}
  ...
  {#if settlement === 'joker'}
    ...
    <div class="mt-2 flex gap-2">
      {#each ['yes', 'no'] as const as side (side)}
        <button class="rounded border p-1.5 {choice === side ? 'border-[#FFD700] bg-[#FFD700]/10' : 'border-[#444] hover:border-[#FFD700]/60'}"
          onclick={() => (choice = side)} title={nameOfCard(side)}>
  ...
  {:else if settlement === 'back'}
    ...
    <div class="mt-2 flex flex-wrap gap-2">
      {#each game.graveyard as card (card)}
        <button class="rounded border p-1.5 {choice === card ? 'border-[#FFD700] bg-[#FFD700]/10' : 'border-[#444] hover:border-[#FFD700]/60'}"
          onclick={() => (choice = card)} title={nameOfCard(card)}>
          <MorenaCard face={card} label={nameOfCard(card)} ink={cssInk(CARD_COLOURS[card])} />
        </button>
      {/each}
    </div>
  ...
  {/if}
  <button class="mt-3 rounded bg-[#d94f68] px-4 py-2 text-sm font-semibold text-[#0b0b0d] disabled:opacity-40"
    disabled={Boolean(settlement) && !choice} onclick={play}>{copy.settle.play}</button>
{/if}
