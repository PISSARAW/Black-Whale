...
>
  <defs>
    <style>
      ...
      .zone {
        fill: rgba(120, 100, 65, 0.1);
        stroke: #827253;
        stroke-width: 2;
        cursor: pointer;
      }
      .zone:hover {
        fill: rgba(120, 100, 65, 0.22);
      }
      .label {
        fill: #fffff0;
  ...
  <text x="500" y="38" class="label" font-size="28" fill="#ffd700"
    >Cha-R-Controlled Warehouse · Tier 5</text
  >
  <text x="500" y="60" class="sub"
    >Loaded goods, including items smuggled through the black market</text
  >
  <g transform="translate(65 90)">
    ...
    <rect
      role="button"
      tabindex="0"
      aria-label="Inspect stored cargo"
      class="zone"
      x="25"
      y="25"
      width="820"
    ...
    {#each Array(24) as _, i (i)}
      <rect
        class="crate"
        x={60 + (i % 6) * 130}
        y={70 + Math.floor(i / 6) * 80}
        width="90"
        height="52"
      />
      <path
        d={`M${60 + (i % 6) * 130} ${70 + Math.floor(i / 6) * 80} l90 52 M${150 + (i % 6) * 130} ${70 + Math.floor(i / 6) * 80} l-90 52`}
        stroke="#665131"
        stroke-width="1"
      />
    {/each}
