...
<aside class="pointer-events-auto absolute left-3 top-14 z-20 text-[10px] text-[#FFFFF0]">
  ...
  {#if open}
    <div class="mt-1 grid w-48 grid-cols-3 gap-1 rounded border border-[#8ecae6]/25 bg-[#050505]/95 p-2 backdrop-blur">
      ...
      <button disabled={!enabled('on')} title={reason('on')} class:active={nenState.on} onclick={() => onAction({ type: 'ON', on: !nenState.on, distribution: { hands: 0.45, torso: 0.35, feet: 0.2 } })}>On</button>
      <button disabled={!enabled('gyo')} title={reason('gyo')} class:active={nenState.gyo} onclick={() => toggle('GYO')}>Gyo</button>
