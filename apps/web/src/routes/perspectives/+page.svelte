...
<div class="max-w-7xl mx-auto p-6">
  <header class="mb-10">
    ...
    <p class="text-gray-400">Explore a character's subjective world at a precise moment, or compare the beliefs of two protagonists.</p>
    <div class="mt-4 flex flex-wrap gap-2 text-xs">
      {#if selectedLeft}
        <a href={`/perspectives/${selectedLeft}`} class="border border-gray-700 rounded px-3 py-1 hover:border-bw-gold">Open subjective map</a>
        <a href={`/knowledge/${selectedLeft}`} class="border border-gray-700 rounded px-3 py-1 hover:border-bw-gold">Open knowledge map</a>
      {/if}
  ...
  {:else if selectedRight && data.comparison}
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div class="bg-bw-dark border border-gray-800 rounded-xl p-6">
        ...
        <h3 class="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">Identity state (V2)</h3>
        <div class="bg-bw-navy/50 p-4 rounded-lg border border-gray-800 mb-6 font-mono text-xs text-gray-300">
          <p>Occupied body: <span class="text-white">{data.leftPerspective?.observer?.currentBodyId || 'Unknown'}</span></p>
          <p>Active consciousness: <span class="text-white">{data.leftPerspective?.observer?.consciousnessId || 'Unknown'}</span></p>
        </div>
        
        <h3 class="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">Subjective facts & beliefs</h3>
        <ul class="space-y-2">
      ...
      <div class="bg-bw-dark border border-gray-800 rounded-xl p-6">
        ...
        <h3 class="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">Identity state (V2)</h3>
        <div class="bg-bw-navy/50 p-4 rounded-lg border border-gray-800 mb-6 font-mono text-xs text-gray-300">
          <p>Occupied body: <span class="text-white">{data.rightPerspective?.observer?.currentBodyId || 'Unknown'}</span></p>
          <p>Active consciousness: <span class="text-white">{data.rightPerspective?.observer?.consciousnessId || 'Unknown'}</span></p>
        </div>
    ...
    <div class="mt-8 bg-bw-navy/30 border border-bw-gold/30 rounded-xl p-6">
      ...
      <div class="space-y-3">
            ...
            <div class="flex flex-col">
              ...
              <span class="text-sm text-white font-medium">
                {#if diff.differenceType === 'LEFT_ONLY'}
                  {getCharacterName(selectedLeft)} knows this, but {getCharacterName(selectedRight)} does not.
                {:else if diff.differenceType === 'RIGHT_ONLY'}
                  {getCharacterName(selectedRight)} knows this, but {getCharacterName(selectedLeft)} does not.
        ...
        {#if data.comparison.length === 0}
          <p class="text-gray-400 text-sm italic">No differences found in the information known by both characters.</p>
        {/if}
      </div>
    </div>
      ...
      <div class="mb-8">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="bg-bw-navy p-4 rounded-lg border border-gray-800">
            ...
            <span class="font-mono text-white text-sm">{data.leftPerspective.observer?.currentBodyId || 'Not found'}</span>
          </div>
          <div class="bg-bw-navy p-4 rounded-lg border border-gray-800">
            <span class="block text-xs text-gray-500 mb-1">Consciousness</span>
            <span class="font-mono text-white text-sm">{data.leftPerspective.observer?.consciousnessId || 'Not found'}</span>
          </div>
        </div>
      </div>
          ...
          {#each data.leftPerspective.knownFacts || [] as fact}
            <div class="p-4 bg-gray-900 rounded-lg border border-gray-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                ...
                <p class="text-gray-500 text-xs mt-1">Subject: <span class="font-mono">{fact.subjectId}</span></p>
              </div>
