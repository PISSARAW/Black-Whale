...
export const hatsuStatusFr: HatsuStatusMessages = {
  ...
  tokens: {
    forgeries: ['ACCÈS OFFICIEL', 'CASIER VIERGE', 'IDENTITÉ AUTORISÉE', 'SURFACE VIDE'],
    forgeryAria: (a0: string) => `${a0} — contrefaçon Texture Surprise`,
    tribunalCards: [
      'BLEU · ADMISSION',
      'JAUNE · AVERTISSEMENT',
      'JAUNE · CONTRAINTE',
    ...
    light: 'LUMIÈRE',
    lightSeasonal: 'LUMIÈRE ++',
    fire: 'FEU',
    fireSeasonal: 'FEU ++',
    noInvocation: 'AUCUNE INVOCATION',
    twoOnly: 'DEUX AU PLUS',
    notSealed: 'NON CLOS',
    ...
    noOwnFuture: 'PAS SON PROPRE AVENIR',
    incomplete: 'INCOMPLET',
    recalled: 'RAPPELÉ',
    noAura: 'PLUS D’AURA',
    stuck: 'FICHÉ',
    noSurface: 'AUCUNE SURFACE',
    reset: 'CONSUMÉ',
    reattached: 'RECOUSU',
    nothingTorn: 'RIEN D’ARRACHÉ',
    notes: ['DO', 'RÉ', 'MI', 'FA', 'SOL', 'LA', 'SI'],
    levelZero: 'NIV 0',
    nothingToRead: 'RIEN À DÉCODER',
    countered: 'CONTRÉ',
    cleaned: 'NETTOYÉ',
    nenTrap: 'PIÈGE DE NEN',
    ...
    reward: 'RÉCOMPENSE',
    zetsu: 'ZETSU',
    noJurisdiction: 'AUCUNE COMPÉTENCE',
    leftHand: 'MAIN GAUCHE',
    notMoved: 'NON DÉPLACÉ',
    trapDoor: 'PORTE PIÉGÉE',
    returnDoor: 'PORTE DE RETOUR',
    intoHideout: 'VERS LA PLANQUE',
    backToRoom: 'RETOUR EN 3101',
    obeyed: 'OBÉI',
    taken: 'PRIS',
    met: (a0: number) => `SATISFAIT ${a0}/3`,
    unmet: 'NON SATISFAIT',
    trueAnswer: 'VRAI',
    route: 'ITINÉRAIRE',
    reveal: 'RÉVÉLATION',
    inert: 'INERTE',
    taboo: 'TABOU',
    trapSprung: 'PIÈGE DÉCLENCHÉ',
    tooTired: 'TROP ÉPUISÉE',
    yes: 'OUI',
    protectedRoom: 'SALLE 1013',
    emptyCopy: 'COPIE VIDE',
    notConnected: 'AUCUN LIEN',
    ren: (a0: number) => `REN ${a0}`,
    charge: (a0: string) => `CHARGE · ${a0}`,
    answered: (a0: number) => `RÉPONSES ×${a0}`,
    grow: (a0: number) => `CROISSANCE ${a0}`,
    victim: (a0: string) => `VICTIME · ${a0}`,
    predicted: (a0: string) => `PRÉDIT · ${a0}`,
    small: (a0: string) => `PETITE FORME · ${a0}`,
    trueForm: (a0: string) => `VRAIE FORME · ${a0}`,
    armed: (a0: string) => `ARMÉ · ${a0}`,
    bite: (a0: number) => `MORSURE ${a0}`,
    wrapped: (a0: string) => `ENVELOPPÉ · ${a0}`,
    released: (a0: string) => `RESSORTI · ${a0}`,
    chargeLevel: (a0: number) => `CHARGE ${a0}`,
    order: (a0: string) => `ORDRE · ${a0}`,
    pass: (a0: number) => `PASSAGE ${a0}`,
    doors: (a0: number) => `${a0} SORTIES`,
    sewn: (a0: number) => `COUSU ${a0} px`,
    slack: (a0: number) => `RELÂCHÉ ${a0} px`,
    level: (a0: string | number) => `NIV ${a0}`,
    levelGain: (a0: number, a1: number) => `+${a0} → NIV ${a1}`,
    hit: (a0: number) => `COUP ×${a0}`,
    abilities: (a0: number) => `${a0} POUVOIRS`,
    read: (a0: number) => `LECTURE ${a0}/3`,
    reach: (a0: number) => `PORTÉE ${a0}`,
    freed: (a0: string) => `LIBÉRÉ · ${a0}`,
    coil: (a0: number) => `ANNEAU ${a0}`,
    relay: (a0: number) => `RELAIS ${a0}`,
    healed: (a0: string) => `GUÉRI · ${a0}`,
    mending: (a0: number) => `SUTURE ${a0}/2`,
    heart: (a0: string) => `CŒUR · ${a0}`,
    rule: (a0: number) => `RÈGLE ${a0}`,
    spentAbility: (a0: string) => `ÉPUISÉ · ${a0}`,
    sign: (a0: number) => `SIGNATURE ${a0}/2`,
    drop: (a0: number) => `GOUTTE ${a0}`,
    hideout: (a0: string) => `PLANQUE · ${a0}`,
    guardLevel: (a0: number) => `GARDE ${a0}`,
    sink: (a0: string) => `RECEVEUR · ${a0}`,
    lie: (a0: number) => `MENSONGE ${a0}`,
    partner: (a0: string) => `PARTENAIRE · ${a0}`,
    bait: (a0: string) => `APPÂT · ${a0}`,
    inhaling: (a0: number) => `${a0} RESPIRENT`,
