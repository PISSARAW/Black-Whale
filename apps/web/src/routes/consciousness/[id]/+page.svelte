...
<div class="max-w-4xl mx-auto p-6 space-y-4">
  <header class="bw-panel p-5">
    <h1 class="font-condensed text-3xl text-[#e5c57a]">Consciousness history: {id}</h1>
    <p class="text-sm text-slate-300 mt-2">Tracking transfers, suppressions, and mental anchors.</p>
  </header>

  <section class="bw-panel p-4">
    <ol class="space-y-3">
      {#each transfers as step}
        <li class="border border-slate-700 rounded p-3">
          <p class="text-xs uppercase tracking-wider text-slate-400">Event {step.event}</p>
          <p class="text-sm text-slate-100">{step.from} -> {step.to}</p>
          <p class="text-xs text-slate-400 mt-1">Certainty: {step.confidence}</p>
        </li>
      {/each}
    </ol>
  </section>
