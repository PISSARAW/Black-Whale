<script lang="ts">
  ...
  let eventCount = $derived(data.chapters.reduce((total, chapter) => total + chapter.events.length, 0));
...
<style>
  ...
  .eyebrow, aside > p, .chapter-header p {
    margin: 0 0 .55rem;
    color: #c9a44a;
    font-size: .66rem;
    font-weight: 700;
  ...
  .stats dt { color: #70817f; font-size: .58rem; font-weight: 700; letter-spacing: .12em; text-transform: uppercase; }
  .stats dd { margin: .2rem 0 0; color: #e6dfc8; font: 600 1.35rem/1 'IBM Plex Sans Condensed', sans-serif; }

  .toolbar { display: flex; position: sticky; z-index: 20; top: .75rem; max-width: 1180px; margin: 0 auto 2.4rem; align-items: center; justify-content: space-between; gap: 1rem; padding: .65rem; border: 1px solid rgba(105,128,137,.24); border-radius: .8rem; background: rgba(8,14,20,.88); box-shadow: 0 16px 38px rgba(0,0,0,.22); backdrop-filter: blur(16px); }
  .search-field { display: flex; min-width: min(480px, 100%); align-items: center; gap: .65rem; padding: .25rem .55rem; }
  .search-field svg { width: 1rem; fill: none; stroke: #718587; stroke-width: 1.7; }
  ...
  .canon-badge span { font-size: .45rem; }

  .timeline-layout { display: grid; max-width: 1180px; margin: 0 auto; grid-template-columns: 76px minmax(0, 1fr); gap: clamp(1.5rem, 4vw, 4rem); }
  aside { position: sticky; top: 6.5rem; height: fit-content; }
  aside nav { display: grid; gap: .35rem; }
  aside a { display: flex; align-items: baseline; gap: .25rem; padding: .38rem .45rem; border-left: 1px solid #2b3a43; color: #7f8f91; text-decoration: none; transition: .18s ease; }
  aside a span { font-size: .5rem; letter-spacing: .08em; }
  aside a strong { font: 500 .78rem/1 'IBM Plex Sans Condensed', sans-serif; }
  aside a:hover { border-color: #c9a44a; color: #e3c66f; background: rgba(201,164,74,.05); }

  .timeline { min-width: 0; }
  .chapter { display: grid; grid-template-columns: 28px minmax(0, 1fr); gap: clamp(1rem, 2.5vw, 2rem); scroll-margin-top: 7rem; }
  .rail { display: flex; align-items: center; flex-direction: column; }
  .chapter-dot { width: 13px; height: 13px; flex: none; margin-top: .45rem; border: 3px solid #071019; border-radius: 50%; background: #c9a44a; box-shadow: 0 0 0 1px rgba(201,164,74,.65), 0 0 18px rgba(201,164,74,.18); }
  .line { width: 1px; height: 100%; min-height: 5rem; background: linear-gradient(#4c5552, rgba(48,64,70,.25)); }
  .chapter-content { padding-bottom: clamp(3rem, 7vw, 6rem); }
  .chapter-header { display: flex; align-items: end; justify-content: space-between; gap: 1rem; margin-bottom: 1rem; }
  .chapter-header h2 { margin: 0; color: #eff0e8; font-size: clamp(1.65rem, 3vw, 2.35rem); font-weight: 500; line-height: 1; }
  .chapter-header > span { color: #647476; font-size: .68rem; white-space: nowrap; }

  .events { display: grid; gap: .6rem; margin: 0; padding: 0; list-style: none; }
  .events a { display: grid; position: relative; overflow: hidden; grid-template-columns: 42px minmax(0, 1fr) auto; align-items: center; gap: 1rem; padding: 1.15rem 1.2rem; border: 1px solid rgba(91,112,120,.24); border-radius: .68rem; background: linear-gradient(110deg, rgba(17,25,36,.88), rgba(10,18,26,.76)); color: inherit; text-decoration: none; transition: border-color .2s ease, transform .2s ease, background .2s ease; }
  .events a::before { position: absolute; inset: 0 auto 0 0; width: 2px; background: #c9a44a; content: ''; opacity: 0; transition: opacity .2s ease; }
  .events a:hover { transform: translateX(4px); border-color: rgba(201,164,74,.38); background: linear-gradient(110deg, rgba(25,32,39,.96), rgba(12,23,32,.88)); }
  ...
  .event-action svg { width: 1rem; fill: none; stroke: #b79a4e; stroke-width: 1.7; transition: transform .2s ease; }
  .events a:hover .event-action svg { transform: translateX(3px); }

  .empty-state { padding: 5rem 1rem; border: 1px dashed rgba(105,128,137,.3); border-radius: .8rem; text-align: center; }
  .empty-state > span { color: #c9a44a; font-size: 2rem; }
  .empty-state h2 { margin: .7rem 0 .25rem; color: #e5e7df; }
  .empty-state p, .results-count { color: #738184; font-size: .78rem; }
  .empty-state button { margin-top: 1rem; padding: .55rem .8rem; border: 1px solid rgba(201,164,74,.35); border-radius: .4rem; background: rgba(201,164,74,.08); color: #ddc273; cursor: pointer; }
  .results-count { margin: -3.5rem 0 0 60px; }
  ...
  @media (max-width: 620px) {
    .timeline-page { padding-inline: .8rem; }
    .toolbar { align-items: stretch; flex-direction: column; }
    .spoiler-badge, .canon-badge { width: fit-content; }
    .timeline-layout { grid-template-columns: 1fr; }
    aside { display: none; }
    .chapter { grid-template-columns: 18px minmax(0, 1fr); gap: .65rem; }
    .events a { grid-template-columns: minmax(0, 1fr) auto; padding: 1rem; }
    .event-index { display: none; }
    .event-action > span { display: none; }
    .event-summary { font-size: .74rem; }
    .stats div { padding: .7rem; }
    .stats dt { font-size: .5rem; }
    .stats dd { font-size: 1.1rem; }
  }

  @media (prefers-reduced-motion: reduce) {
    :global(html) { scroll-behavior: auto; }
    .events a, .event-action svg { transition: none; }
  }
</style>
