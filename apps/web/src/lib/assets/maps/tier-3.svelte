<script lang="ts">
  ...
  function handleZoneKeydown(event: KeyboardEvent, zoneId: string) {
    if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); event.stopPropagation(); handleZoneClick(zoneId); }
  }
</script>
...
<svg viewBox="0 0 1000 600" class="w-full h-full text-[#FFFFF0]">
  ...
  <g id="tier-3-zones">
    ...
    <g id="t3-residential-1st">
      <rect class="zone" role="button" tabindex="0" aria-label="Open First-class Residential Zone" class:selected={mapState.selectedLocationId === 't3-residential-1st'} x="180" y="100" width="180" height="150" onclick={() => handleZoneClick('t3-residential-1st')} onkeydown={(event) => handleZoneKeydown(event, 't3-residential-1st')} />
      <text x="270" y="160" class="label">First-Class Cabins</text>
      
      <!-- Chambre 3101 -->
      <g role="button" tabindex="0" aria-label="Open Room 3101" onclick={(e) => { e.stopPropagation(); handleZoneClick('room-3101'); }} onkeydown={(event) => handleZoneKeydown(event, 'room-3101')}>
    ...
    <g id="t3-residential-ord" role="button" tabindex="0" aria-label="Open Standard Residential Zone" onclick={() => handleZoneClick('t3-residential-ord')} onkeydown={(event) => handleZoneKeydown(event, 't3-residential-ord')}>
    ...
    <g id="t3-hospital" role="button" tabindex="0" aria-label="Open Medical Ward" onclick={() => handleZoneClick('t3-hospital')} onkeydown={(event) => handleZoneKeydown(event, 't3-hospital')}>
    ...
    <g id="t3-political-ward">
      ...
      <g role="button" tabindex="0" aria-label="Open Central Police Station" onclick={() => handleZoneClick('central-police-station')} onkeydown={(event) => handleZoneKeydown(event, 'central-police-station')}>
    ...
    <!-- Loisirs & Heil-Ly -->
    <g id="t3-cinema" role="button" tabindex="0" aria-label="Open Movie Theater" onclick={() => handleZoneClick('t3-cinema')} onkeydown={(event) => handleZoneKeydown(event, 't3-cinema')}>
    ...
    <g id="t3-obs-deck" role="button" tabindex="0" aria-label="Open Observation Deck" onclick={() => handleZoneClick('t3-obs-deck')} onkeydown={(event) => handleZoneKeydown(event, 't3-obs-deck')}>
    ...
    <g id="t3-heilly" role="button" tabindex="0" aria-label="Open Heil-Ly Office" onclick={() => handleZoneClick('t3-heilly')} onkeydown={(event) => handleZoneKeydown(event, 't3-heilly')}>
