...
export function debriefAxes(state: InfiltrationState): DebriefAxis {
  ...
  const material = required.every((objective) => objective.state === 'believed' || objective.state === 'confirmed') ? 'complete' : 'incomplete'
  const informed = state.objectives.filter((objective) => objective.kind === 'identify')
  const information = informed.length === 0 || informed.some((objective) => objective.state === 'confirmed') ? 'true' : informed.some((objective) => objective.state === 'invalidated') ? 'false' : 'uncertain'
  return { material, information, cover: state.alertLevel === 'identified' || state.coverIntegrity === 0 ? 'compromised' : 'intact' }
}
