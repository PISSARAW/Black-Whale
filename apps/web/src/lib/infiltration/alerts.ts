...
export function assessAlert(reports: { witnessId: string; certainty: number }[], compromised: number): AlertAssessment {
  if (reports.some((report) => report.certainty >= 95)) return { level: 'identified', cause: 'confirmed-identity' }
  if (new Set(reports.map((report) => report.witnessId)).size >= 2) return { level: 'lockdown', cause: 'corroborated-reports' }
