...
<nav
  class="absolute bottom-16 left-3 z-20 flex gap-2 sm:bottom-6 sm:left-6"
  aria-label={fr() ? 'Nen avancé' : 'Advanced Nen'}
>
  ...
  <button
    ...
    class:active={state.shuItem !== null}
    disabled={!canShu}
    onclick={onShu}
  >
  ...
  {#if state.wounds.length > 0}
    <span
      class="rounded-full border border-rose-300/30 bg-black/70 px-3 py-2 text-xs text-rose-200"
    >
...
<style>
  ...
  .advanced-action.active {
    border-color: rgb(250 204 21 / 0.65);
    color: white;
  }
  .advanced-action:focus-visible {
    outline: 2px solid white;
    outline-offset: 2px;
  }
  .advanced-action:disabled {
    opacity: 0.3;
  }
  kbd {
    margin-right: 0.4rem;
