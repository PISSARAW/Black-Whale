...
export type InfiltrationHatsuId =
  | 'little-eye' | 'texture-surprise' | 'illumi-needle-people'
  | 'secret-window' | 'biohazard-hinrigh' | 'surveillance-paper-dolls'
  | 'bloody-mary' | 'body-and-soul' | 'dowsing-chain' | 'blinky'
  | 'bungee-gum' | 'skill-hunter' | 'stealth-dolphin'
export type HatsuRole = 'scout' | 'forge' | 'disguise' | 'surveillance' | 'tracker' | 'interrogate' | 'analyse' | 'cleanup' | 'mobility' | 'theft'
...
export interface HatsuPlan {
  available: boolean
  conditions: { id: 'ten' | 'conscious' | 'aura' | 'uses' | 'uninterrupted' | 'target'; met: boolean }[]
...
export const INFILTRATION_HATSU: InfiltrationHatsu[] = [
  ...
  fromProfile(hatsuById('secret-window')!, { actionId: 'attach-owl', cost: 20, uses: 1, role: 'surveillance' }),
  fromProfile(hatsuById('biohazard-hinrigh')!, { actionId: 'animate-camera', cost: 24, uses: 2, role: 'scout' }),
  fromProfile(hatsuById('surveillance-paper-dolls')!, { actionId: 'paper-spy', cost: 12, uses: 3, role: 'surveillance' }),
  fromProfile(hatsuById('bloody-mary')!, { actionId: 'blood-search', cost: 16, uses: 2, role: 'tracker' }),
  fromProfile(hatsuById('body-and-soul')!, { actionId: 'truth-punch', cost: 28, uses: 1, role: 'interrogate' }),
  fromProfile(hatsuById('dowsing-chain')!, { actionId: 'dowsing', cost: 14, uses: 2, role: 'analyse' }),
  fromProfile(hatsuById('blinky')!, { actionId: 'vacuum', cost: 25, uses: 2, role: 'cleanup' }),
  fromProfile(hatsuById('bungee-gum')!, { actionId: 'attach', cost: 15, uses: 2, role: 'mobility' }),
  fromProfile(hatsuById('skill-hunter')!, { actionId: 'borrow', cost: 32, uses: 1, role: 'theft' }),
  fromProfile(hatsuById('stealth-dolphin')!, { actionId: 'loan', cost: 30, uses: 1, role: 'theft' }),
]
...
export function castHatsu(state: InfiltrationState): InfiltrationState {
  ...
  if (ability.role === 'forge') {
    ...
    return {
      ...state,
      cover,
      hatsu: { ...hatsu, forgedOrder: true, effect: { kind: 'forged-surface', spaceId: state.player.spaceId ?? state.extractionSpaceId, payload: state.hatsu.forgerySurface } },
      traces: [
        ...state.traces,
  ...
  const nearest = state.witnesses.find((witness) => witness.id === state.hatsu.targetWitnessId) ?? [...state.witnesses].sort(
    (a, b) => Math.hypot(a.position[0] - state.player.position[0], a.position[1] - state.player.position[1]) - Math.hypot(b.position[0] - state.player.position[0], b.position[1] - state.player.position[1]),
  )[0]
  if (ability.id === 'secret-window') return { ...state, hatsu: { ...hatsu, effect: { kind: 'attached-owl', witnessId: nearest?.id, expiresAt: state.clock + 150 } } }
  if (ability.id === 'surveillance-paper-dolls') return { ...state, hatsu: { ...hatsu, effect: { kind: 'paper-network', spaceId: activeSpaceId, expiresAt: state.clock + 180 } }, traces: [...state.traces, createTrace({ kind: 'forgery', spaceId: activeSpaceId, position: state.player.position, at: state.clock, strength: 12, duration: 180, allegedAuthor: 'paper-spy' })] }
  if (ability.id === 'bloody-mary') return { ...state, hatsu: { ...hatsu, effect: { kind: 'blood-tracker', witnessId: nearest?.id, expiresAt: state.clock + 120 } }, traces: [...state.traces, createTrace({ kind: 'aura', spaceId: activeSpaceId, position: state.player.position, at: state.clock, strength: 16, duration: 75, allegedAuthor: 'blood-user' })] }
  if (ability.id === 'body-and-soul') return { ...state, authorConfirmed: true, coverIntegrity: Math.max(0, state.coverIntegrity - 35), alert: Math.min(100, state.alert + 28), hatsu: { ...hatsu, effect: { kind: 'forced-answer', witnessId: nearest?.id, payload: 'truth', expiresAt: state.clock + 2 } } }
  if (ability.id === 'dowsing-chain') return { ...state, authorConfirmed: true, hatsu: { ...hatsu, effect: { kind: 'dowsing-result', spaceId: state.objectiveSpaceId, payload: 'coherent', expiresAt: state.clock + 12 } } }
  if (ability.id === 'blinky') {
    const removable = state.traces.filter((trace) => trace.spaceId === activeSpaceId && trace.kind !== 'aura')
    return { ...state, traces: state.traces.filter((trace) => !removable.includes(trace)), hatsu: { ...hatsu, effect: { kind: 'cleaned', spaceId: activeSpaceId, payload: `${removable.length}`, expiresAt: state.clock + 2 } } }
  }
  if (ability.id === 'bungee-gum') return { ...state, hatsu: { ...hatsu, effect: { kind: 'gum-anchor', spaceId: activeSpaceId, expiresAt: state.clock + 90 } }, diversion: { spaceId: activeSpaceId, left: 12 } }
  if (ability.id === 'skill-hunter' || ability.id === 'stealth-dolphin') return { ...state, authorConfirmed: true, hatsu: { ...hatsu, effect: { kind: ability.id === 'skill-hunter' ? 'borrowed-page' : 'loaned-ability', payload: 'dowsing-chain', expiresAt: state.clock + 15 } } }
  return {
    ...state,
    hatsu: { ...hatsu, activeUntil: state.clock + 75, effect: { kind: 'disguise-mask', spaceId: activeSpaceId, payload: state.hatsu.disguiseIdentity, expiresAt: state.clock + 75 } },
    traces: [
      ...state.traces,
...
export function moveLittleEye(state: InfiltrationState, position: Vec2, spaceId: string, visibleToGuard: boolean): InfiltrationState {
  if ((state.hatsu.id !== 'little-eye' && state.hatsu.id !== 'biohazard-hinrigh') || !state.hatsu.scout?.active) return state
