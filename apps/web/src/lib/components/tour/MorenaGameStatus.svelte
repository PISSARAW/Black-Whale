<script lang="ts">
  ...
  const verse = $derived(
    losing ? ((copy.hatsu.ghost.verse as Record<string, string[]>)[losing] ?? []) : [],
  )
...
{#if game.phase !== 'over'}
  <p class="text-[10px] uppercase tracking-widest text-[#FFD700]/70" aria-live="polite">
    {copy.round(game.asked.length, game.hand.length)}
  </p>
{/if}
...
{#if game.manipulated}
  <div class="mt-3 rounded border border-[#9d65d0]/70 bg-[#9d65d0]/10 p-3">
    ...
    <p class="mt-1 text-xs leading-relaxed text-[#FFFFF0]/80">
      {game.ending === 'abandoned' ? copy.hatsu.narrowed.leaving : copy.hatsu.narrowed.cheating}
    </p>
  </div>
{/if}
