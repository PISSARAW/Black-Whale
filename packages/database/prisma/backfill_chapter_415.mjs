...
async function main() {
	const [flashback, message, camps, detention, room1003, room1004, room1005, room1006, room1011, room1013, room1014, royalSector] = await Promise.all([
	...
	const slugs = [
		'furykov', 'beyond-netero', 'kurapika', 'queen-oito', 'bill', 'babimyna',
		'prince-zhanglei', 'coventoba', 'prince-tubeppa', 'maor', 'rihan', 'theta', 'salkov',
		'prince-tyson', 'prince-fugetsu',
		'biscuit-krueger', 'vergei', 'prince-marayam', 'ridge', 'kanjidol'
	];
	const characters = new Map((await Promise.all(slugs.map(requiredCharacter))).map((character) => [character.slug, character]));

	const participationGroups = [
		[flashback, [['furykov', 'ACTIVE'], ['beyond-netero', 'ACTIVE']]],
		[message, [['kurapika', 'ACTIVE'], ['queen-oito', 'ACTIVE'], ['bill', 'PASSIVE'], ['babimyna', 'OBSERVER']]],
		[camps, [
			['prince-zhanglei', 'PASSIVE'], ['coventoba', 'OBSERVER'], ['prince-tubeppa', 'PASSIVE'],
			['maor', 'OBSERVER'], ['rihan', 'OBSERVER'], ['theta', 'OBSERVER'], ['salkov', 'OBSERVER'],
			['prince-tyson', 'PASSIVE'], ['prince-fugetsu', 'PASSIVE'], ['biscuit-krueger', 'OBSERVER'],
			['vergei', 'OBSERVER'], ['prince-marayam', 'PASSIVE']
		]],
		[detention, [['ridge', 'ACTIVE'], ['kanjidol', 'VICTIM']]]
	];
	for (const [event, participants] of participationGroups) {
		for (const [slug, type] of participants) await upsertParticipation(event, characters.get(slug), type);
	}

	await prisma.narrativeEvent.update({ where: { id: message.id }, data: { locationId: room1014.id } });
	await prisma.narrativeEvent.update({ where: { id: camps.id }, data: { locationId: royalSector.id } });
	await prisma.narrativeEvent.update({ where: { id: detention.id }, data: { locationId: royalSector.id } });
