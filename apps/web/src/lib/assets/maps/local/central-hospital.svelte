...
<svg viewBox="0 0 1000 680" class="w-full h-full text-[#FFFFF0] bg-[#050505] rounded-lg border border-[#333]">
  <defs>
    <style>
      .wall { fill: none; stroke: #eaf7ff; stroke-width: 5; }
      .zone { fill: rgba(70,150,185,.09); stroke: #6097ac; stroke-width: 2; cursor: pointer; }
      .zone:hover { fill: rgba(70,150,185,.2); }
      .label { fill: #fffff0; font: bold 15px sans-serif; text-anchor: middle; pointer-events: none; }
      .sub { fill: #8ec4d9; font: 11px sans-serif; text-anchor: middle; pointer-events: none; }
      .bed { fill: #dcebf1; stroke: #6b8792; stroke-width: 2; }
    </style>
  </defs>
  ...
  <g transform="translate(60 90)">
    ...
    {#each [0,1,2,3] as i}
      <rect class="bed" x={75 + (i % 2) * 240} y={115 + Math.floor(i / 2) * 95} width="150" height="55" rx="6" />
    {/each}
    <rect role="button" tabindex="0" aria-label="Inspect medical supplies" class="zone" x="555" y="25" width="300" height="145" onclick={() => inspect('medical-supplies')} onkeydown={(event) => inspectWithKeyboard(event, 'medical-supplies')} />
