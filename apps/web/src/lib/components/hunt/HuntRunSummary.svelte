...
<section class="mt-8 rounded-xl border border-white/10 bg-white/[0.025] p-4">
  ...
  <dl class="mt-3 grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
    <div><dt class="text-white/40">{labels.rooms}</dt><dd>{metrics.roomsVisited}</dd></div>
    <div><dt class="text-white/40">{labels.zetsu}</dt><dd>{Math.round(metrics.timeInZetsu)} s</dd></div>
    <div><dt class="text-white/40">{labels.hatsu}</dt><dd>{metrics.hatsuUses}</dd></div>
    <div><dt class="text-white/40">{labels.falseTrails}</dt><dd>{metrics.falseTrails}</dd></div>
  </dl>
