 ...
 * say no.
 *
 * Everything here is pure and total. `castInTour` is the model: state in, state
 * out, chance injected as `random` so a test can settle a deal it did not
 * shuffle. Nothing in this file knows about three.js, and nothing in it knows
 * about the message catalogue either — a card is an id, and what Morena *says*
 * when she answers a question is the route's business.
 *
 * The drawing half lives at the bottom: `tableauOf` turns a game into the same
 * `Apparition` list `$lib/tour/apparitions` produces, so the walk's scene can
 * lay the cards on the table without learning a second vocabulary.
 */
...
} as const satisfies Partial<Record<HatsuInteractionKind, TableMove>>
...
export function tableauOf(game: MorenaGame, floor: number): Apparition[] {
  ...
  /** A card, laid flat, spread about the middle of one side of the table. */
  const lay = (
    id: string,
    index: number,
    count: number,
    ...
    seen.push({
      ...
      id,
      kind: 'game-card',
      at: [TABLE_AT[0] + offset, TABLE_AT[1] + depth],
      y: top + 0.01 + lift,
      size: 0.11,
      colour,
      stage,
  ...
  game.hand.forEach((card, index) => {
    ...
    lay(
      ...
      CARD_COLOURS[card],
      nicked ? 3 : 1,
      game.phase === 'settling' || game.phase === 'over' ? 0.02 : 0,
    )
  })
