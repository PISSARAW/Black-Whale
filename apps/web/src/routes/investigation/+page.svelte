...
<main class="min-h-screen bg-[#020617] px-4 py-10 text-slate-100 sm:px-8 lg:px-12 relative overflow-hidden">
  <!-- Ethereal Background glow -->
  <div class="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-sky-900/20 via-[#020617] to-[#020617]"></div>
  ...
  <div class="relative mx-auto max-w-6xl z-10">
    <div class="mb-12 text-center md:text-left">
      <p class="inline-flex items-center gap-2 rounded-full border border-sky-400/30 bg-sky-900/30 px-4 py-1 text-[10px] font-bold uppercase tracking-[0.28em] text-sky-300 shadow-[0_0_15px_rgba(56,189,248,0.2)]">
        <span class="h-1.5 w-1.5 rounded-full bg-sky-300 shadow-[0_0_8px_#7dd3fc]"></span>
        {copy.eyebrow}
      </p>
      <h1 class="mt-6 text-5xl font-black tracking-tight text-white drop-shadow-[0_0_20px_rgba(255,255,255,0.2)] sm:text-7xl">{copy.title}</h1>
      ...
      {#each cases as investigationCase}
        ...
        >
          ...
          <div class="absolute inset-0 z-0 opacity-0 transition-opacity duration-500 group-hover:opacity-100">
            <div class="absolute inset-0 bg-gradient-to-tr from-sky-400/0 via-sky-400/20 to-emerald-400/0"></div>
          </div>
          
          <div class="relative z-10 flex h-full flex-col rounded-xl border border-white/5 bg-[#0a0f1c]/80 p-6 backdrop-blur-md">
            <div class="flex items-start justify-between gap-4">
              <div>
                <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-sky-400/80 drop-shadow-[0_0_8px_rgba(56,189,248,0.3)]">
              ...
              <div class="relative">
                {#if status === 'solved'}
                  <span class="absolute -inset-1 animate-pulse rounded-full bg-emerald-400/20 blur-sm"></span>
                {/if}
                <span
                  class="relative flex items-center justify-center rounded-full border px-3 py-1 text-[10px] font-bold uppercase tracking-wider {status === 'solved'
            ...
            <div class="mt-6 rounded-lg border border-sky-900/30 bg-sky-950/20 p-4">
              <dl class="grid grid-cols-2 gap-y-4 gap-x-2 text-xs">
                <div>
                  <dt class="text-[9px] uppercase tracking-wider text-sky-500/70">{copy.investigator}</dt>
                  <dd class="mt-1 font-mono text-sky-100">{investigationCase.investigator}</dd>
                </div>
                <div>
                  <dt class="text-[9px] uppercase tracking-wider text-sky-500/70">{copy.chapter}</dt>
                  <dd class="mt-1 font-mono text-sky-100">{investigationCase.requiredChapter}</dd>
                </div>
                <div>
                  <dt class="text-[9px] uppercase tracking-wider text-sky-500/70">{copy.duration}</dt>
                  <dd class="mt-1 font-mono text-sky-100">~{investigationCase.estimatedMinutes}m</dd>
                </div>
                <div>
            ...
            >
              <div class="absolute inset-0 bg-gradient-to-r from-sky-400/0 via-sky-400/10 to-sky-400/0 opacity-0 transition-opacity duration-300 group-hover/btn:opacity-100"></div>
              <span class="relative z-10 flex items-center gap-2">
                {status === 'in-progress' ? copy.resume : copy.open}
                <svg class="h-4 w-4 transition-transform duration-300 group-hover/btn:translate-x-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
              </span>
              <!-- Binder Ring Effect -->
              <div class="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 rounded-full border-2 border-sky-400/30 opacity-0 shadow-[0_0_8px_#38bdf8] transition-all duration-300 group-hover/btn:opacity-100"></div>
              <div class="absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 rounded-full border-2 border-sky-400/30 opacity-0 shadow-[0_0_8px_#38bdf8] transition-all duration-300 group-hover/btn:opacity-100"></div>
            </a>
          </div>
        </article>
      {/each}
