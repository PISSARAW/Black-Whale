# Audit de l’apparence des pièces du Black Whale

> Fichier généré par `node scripts/generate-room-appearance-audit.mjs` à partir de
> `data/ship/blueprint.json`. Il inventorie chaque lieu visitable catalogué, les détails
> effectivement modélisés et la source qui autorise chacun d’eux.

## Comment lire l’audit

- **Case du manga** : forme ou objet directement visible ; c’est l’autorité principale.
- **Plan publié** : emprise ou distribution donnée par un plan, sans inventer un décor.
- **Reconstruction cartographique** : détail repris du plan `/ship`, faute de case complète.
- **Liaison déduite** : uniquement ce qui est nécessaire pour rendre le pont parcourable.
- Une pièce vide dans le manga reste vide ici : une absence de meuble attesté est une information.

**Couverture : 61 lieux, 409 espaces, 972 éléments modélisés.**

## Salle du banquet — `tier-1-banquet-hall`

- **Pont :** tier-1
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 2
- **Emprises relevées :** 133.5 × 24.5 m ; 24 × 24.5 m
- **Hauteurs sous plafond :** 9, 9.6 m
- **Éléments modélisés :** 72 table, 8 platform, 5 counter, 2 painting, 2 pillar, 1 cabinet
- **Objets directement visibles dans les cases :** 90/90
- **Espaces internes :** Salle du banquet (bout du service)
- **Sources de la pièce :**
  - Chap. 349 — cérémonie de départ
  - Chap. 383 — le bout de la halle où le buffet est dressé, une marche en contrebas du sol où se tiennent les tables, le plafond filant de niveau au-dessus des deux.
- **Sources du mobilier et des détails :**
  - Chap. 383 — la scène au bout de la salle, et le trône sur son estrade devant elle.
  - Chap. 383 — le buffet qui sert la salle, les cuisiniers derrière le comptoir et les plats disposés dessus.
  - Chap. 359 — le banquet inaugural : des tables rondes en rangées à travers la salle, une allée laissée libre dans l'axe du trône.
  - Chap. 359 — le banquet inaugural vu dans l'axe de la halle : les galeries surélevées qui courent le long des deux murs latéraux, les piles et le rideau qui encadrent la scène, et le panneau suspendu au-dessus de la galerie bâbord.
  - Chap. 383 — le trône sur son estrade : le parapet drapé en travers de sa face et l'emmarchement qui en descend, et la ligne de service d'où le buffet est dressé, derrière le comptoir.

## Quartiers du Roi — `tier-1-king-living-quarters`

- **Pont :** tier-1, interior-king-quarters
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 2
- **Emprises relevées :** 192.5 × 28 m ; 24 × 16.64 m
- **Hauteurs sous plafond :** 7 m
- **Éléments modélisés :** 9 painting, 6 cabinet, 3 seat, 3 table, 1 platform
- **Objets directement visibles dans les cases :** 22/22
- **Espaces internes :** Salon royal
- **Sources de la pièce :**
  - Chap. 382, p. 29–32 — le salon à double hauteur du Roi : voûte en berceau à caissons sur piles carrées, sol géométrique incrusté, œuvre monumentale et nombreux petits cadres, manteau garni d'objets, sièges rembourrés, tables basses, buffets, plantes, sculpture et desserte de boissons.
- **Sources du mobilier et des détails :**
  - Chap. 382, p. 29–30 — un tableau de bataille monumental domine le mur au-dessus du manteau ornementé.
  - Chap. 382 — le salon du Roi : sol à motifs, sièges appariés, tableaux encadrés et la baie du fond.
  - Chap. 382, p. 29–30 — un long manteau sous le tableau de bataille monumental porte des photographies encadrées, des vases et un ornement central évoquant une horloge.
  - Chap. 382, p. 29–30 — des meubles d'exposition et objets ornementaux bordent les deux côtés du manteau central.
  - Chap. 382, p. 29–30 — une grande enceinte étroite à trois haut-parleurs circulaires se dresse près du mur de tableaux bâbord.
  - Chap. 382, p. 29–30 — bouteilles, verres et récipients couvrent une desserte drapée près du groupe de sièges du Roi.
  - Chap. 413 — Halkenburg exposé dans le salon de son père, le Roi debout à son chevet.

## Pont des canots (bâbord) — `tier-1-lifeboats`

- **Pont :** tier-1, interior-lifeboat
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 3
- **Emprises relevées :** 10.5 × 35 m ; 5.2 × 5.2 m
- **Hauteurs sous plafond :** 2.6 m
- **Éléments modélisés :** 8 lifeboat, 3 counter, 1 cabinet, 1 pillar, 1 platform
- **Objets directement visibles dans les cases :** 14/14
- **Espaces internes :** Pont des canots (tribord) ; Cabine de la capsule
- **Sources de la pièce :**
  - Chap. 383 — la baie de lancement : les capsules sur leurs berceaux et la console de mise à l'eau à côté.
  - Chap. 383, p. 60–65 — la cabine circulaire possède une coque nervurée, une écoutille ronde avec console et meuble latéraux, et un pont nu segmenté autour du mât central et de son large socle ; aucun siège de passager n'est visible.
- **Sources du mobilier et des détails :**
  - Chap. 383 — la baie de lancement : les capsules sur leurs berceaux et la console de mise à l'eau à côté.
  - Chap. 383, p. 65 — le mât central s'élève d'un large socle circulaire sur le pont segmenté nu.
  - Chap. 383, p. 60 — une console d'instruments basse se trouve directement sous l'écoutille ronde ouverte.
  - Chap. 383, p. 60 — un meuble rectangulaire séparé se dresse à droite de la console de l'écoutille.

## Chambre funéraire des princes — `tier-1-princes-burial-chamber`

- **Pont :** tier-1, interior-burial-chamber
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 2
- **Emprises relevées :** 15.92 × 15.36 m ; 20.54 × 20.54 m
- **Hauteurs sous plafond :** 6, 8 m
- **Éléments modélisés :** 17 casket
- **Objets directement visibles dans les cases :** 17/17
- **Espaces internes :** Rotonde
- **Sources de la pièce :**
  - Le chap. 358 montre la chambre ; le chap. 371 la dessine ronde, les quatorze cercueils disposés en rayons autour de son centre.
- **Sources du mobilier et des détails :**
  - Le chap. 358 montre la chambre ; le chap. 371 la dessine ronde, les quatorze cercueils disposés en rayons autour de son centre.

## Couloir des quartiers des reines — `tier-1-queens-living-quarters`

- **Pont :** tier-1-c
- **Preuve de l’enveloppe :** liaison déduite
- **Sous-espaces :** 1
- **Emprises relevées :** 5.95 × 52.5 m
- **Éléments modélisés :** aucun objet non attesté
- **Objets directement visibles dans les cases :** 0/0
- **Sources de la pièce :**
  - Reconstruit pour rendre le pont continu ; aucune planche ne le montre.
- **Limite connue :** 1 liaison(s) ou volume(s) restent déduits pour assurer la continuité de la visite.

## Chambre de reine 01 — `tier-1-queens-living-quarters-room-01`

- **Pont :** tier-1-c
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 1
- **Emprises relevées :** 8.4 × 13.13 m
- **Éléments modélisés :** 3 table, 2 bed, 2 painting, 1 mobile, 1 seat
- **Objets directement visibles dans les cases :** 8/9
- **Sources de la pièce :**
  - Le chap. 403 montre le salon richement lambrissé de la reine Unma et la nursery derrière sa porte ouverte ; le plan des quartiers des reines sur /ship fournit l’emprise de la chambre 01 dans le bloc de huit chambres.
- **Sources du mobilier et des détails :**
  - Le plan des quartiers des reines sur /ship dessine un lit dans chacune des huit chambres, adossé au mur opposé à la porte du couloir.
  - Chap. 403 — la porte ouverte derrière la reine Unma laisse voir un berceau au fond de la chambre 01.
  - Chap. 403 — un mobile suspendu est visible directement au-dessus du berceau dans la nursery de la reine Unma.
  - Chap. 403 — la reine Unma est assise dans un haut fauteuil orné au milieu de son salon richement lambrissé.
  - Chap. 403 — un petit guéridon sculpté se tient de chaque côté du fauteuil de la reine Unma.
  - Chap. 403 — une table basse rectangulaire sculptée se tient devant le fauteuil de la reine Unma.
  - Chap. 403 — des tableaux encadrés sont suspendus dans les panneaux moulurés derrière le fauteuil de la reine Unma.

## Chambre de reine 02 — `tier-1-queens-living-quarters-room-02`

- **Pont :** tier-1-c
- **Preuve de l’enveloppe :** reconstruction cartographique
- **Sous-espaces :** 1
- **Emprises relevées :** 8.4 × 13.13 m
- **Éléments modélisés :** 1 bed
- **Objets directement visibles dans les cases :** 0/1
- **Sources de la pièce :**
  - Le plan des quartiers des reines sur /ship dessine le bloc en huit chambres ; leur ordre le long du couloir est celui de la reconstruction.
- **Sources du mobilier et des détails :**
  - Le plan des quartiers des reines sur /ship dessine un lit dans chacune des huit chambres, adossé au mur opposé à la porte du couloir.

## Chambre de reine 03 — `tier-1-queens-living-quarters-room-03`

- **Pont :** tier-1-c
- **Preuve de l’enveloppe :** reconstruction cartographique
- **Sous-espaces :** 1
- **Emprises relevées :** 8.4 × 13.12 m
- **Éléments modélisés :** 1 bed
- **Objets directement visibles dans les cases :** 0/1
- **Sources de la pièce :**
  - Le plan des quartiers des reines sur /ship dessine le bloc en huit chambres ; leur ordre le long du couloir est celui de la reconstruction.
- **Sources du mobilier et des détails :**
  - Le plan des quartiers des reines sur /ship dessine un lit dans chacune des huit chambres, adossé au mur opposé à la porte du couloir.

## Chambre de reine 04 — `tier-1-queens-living-quarters-room-04`

- **Pont :** tier-1-c
- **Preuve de l’enveloppe :** reconstruction cartographique
- **Sous-espaces :** 1
- **Emprises relevées :** 8.4 × 13.12 m
- **Éléments modélisés :** 1 bed
- **Objets directement visibles dans les cases :** 0/1
- **Sources de la pièce :**
  - Le plan des quartiers des reines sur /ship dessine le bloc en huit chambres ; leur ordre le long du couloir est celui de la reconstruction.
- **Sources du mobilier et des détails :**
  - Le plan des quartiers des reines sur /ship dessine un lit dans chacune des huit chambres, adossé au mur opposé à la porte du couloir.

## Chambre de reine 05 — `tier-1-queens-living-quarters-room-05`

- **Pont :** tier-1-c
- **Preuve de l’enveloppe :** reconstruction cartographique
- **Sous-espaces :** 1
- **Emprises relevées :** 8.4 × 13.13 m
- **Éléments modélisés :** 1 bed
- **Objets directement visibles dans les cases :** 0/1
- **Sources de la pièce :**
  - Le plan des quartiers des reines sur /ship dessine le bloc en huit chambres ; leur ordre le long du couloir est celui de la reconstruction.
- **Sources du mobilier et des détails :**
  - Le plan des quartiers des reines sur /ship dessine un lit dans chacune des huit chambres, adossé au mur opposé à la porte du couloir.

## Chambre de reine 06 — `tier-1-queens-living-quarters-room-06`

- **Pont :** tier-1-c
- **Preuve de l’enveloppe :** reconstruction cartographique
- **Sous-espaces :** 1
- **Emprises relevées :** 8.4 × 13.13 m
- **Éléments modélisés :** 1 bed
- **Objets directement visibles dans les cases :** 0/1
- **Sources de la pièce :**
  - Le plan des quartiers des reines sur /ship dessine le bloc en huit chambres ; leur ordre le long du couloir est celui de la reconstruction.
- **Sources du mobilier et des détails :**
  - Le plan des quartiers des reines sur /ship dessine un lit dans chacune des huit chambres, adossé au mur opposé à la porte du couloir.

## Chambre de reine 07 — `tier-1-queens-living-quarters-room-07`

- **Pont :** tier-1-c
- **Preuve de l’enveloppe :** reconstruction cartographique
- **Sous-espaces :** 1
- **Emprises relevées :** 8.4 × 13.12 m
- **Éléments modélisés :** 1 bed
- **Objets directement visibles dans les cases :** 0/1
- **Sources de la pièce :**
  - Le plan des quartiers des reines sur /ship dessine le bloc en huit chambres ; leur ordre le long du couloir est celui de la reconstruction.
- **Sources du mobilier et des détails :**
  - Le plan des quartiers des reines sur /ship dessine un lit dans chacune des huit chambres, adossé au mur opposé à la porte du couloir.

## Chambre de reine 08 — `tier-1-queens-living-quarters-room-08`

- **Pont :** tier-1-c
- **Preuve de l’enveloppe :** reconstruction cartographique
- **Sous-espaces :** 1
- **Emprises relevées :** 8.4 × 13.12 m
- **Éléments modélisés :** 1 bed
- **Objets directement visibles dans les cases :** 0/1
- **Sources de la pièce :**
  - Le plan des quartiers des reines sur /ship dessine le bloc en huit chambres ; leur ordre le long du couloir est celui de la reconstruction.
- **Sources du mobilier et des détails :**
  - Le plan des quartiers des reines sur /ship dessine un lit dans chacune des huit chambres, adossé au mur opposé à la porte du couloir.

## Chemin de ronde des gardes (bâbord) — `tier-1-royal-residential-sector`

- **Pont :** tier-1
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 32
- **Emprises relevées :** 3 × 49 m ; 49 × 3.5 m ; 3 × 5 m ; 43 × 1.75 m
- **Éléments modélisés :** 29 duct
- **Objets directement visibles dans les cases :** 29/29
- **Espaces internes :** Chemin de ronde des gardes (tribord) ; Chemin de ronde des gardes (arrière) ; Dégagement — porte 1 ; Dégagement — porte 2 ; Dégagement — porte 3 ; Dégagement — porte 4 ; Dégagement — porte 5 ; Dégagement — porte 6 ; Dégagement — porte 7 ; Dégagement — porte 8 ; Dégagement — porte 9 ; Dégagement — porte 10 ; Dégagement — porte 11 ; Dégagement — porte 12 ; Dégagement — porte 13 ; Dégagement — porte 14 ; Traverse 1 ; Traverse 2 ; Traverse 3 ; Traverse 4 ; Traverse 5 ; Traverse 6 ; Traverse 7 ; Traverse 8 ; Coursive arrière 1 ; Coursive arrière 2 ; Coursive arrière 3 ; Coursive arrière 4 ; Coursive arrière 5 ; Coursive arrière 6 ; Coursive arrière 7
- **Sources de la pièce :**
  - Chap. 363 — la double page de la zone d'habitation dessine deux cloisons : l'extérieure, en dedans de laquelle les gardes font leur ronde, et l'intérieure, qui porte les quatorze portes numérotées. Voici le chemin de ronde entre les deux, et ce sont les soldats du chap. 349 qui s’y tiennent.
  - Chap. 363 — les appartements se dressent isolés à l'intérieur de la cloison intérieure, et chaque porte numérotée fait face à la porte de l’appartement qui lui est assigné. Voici le dégagement entre les deux.
  - Chap. 363 — les quatorze appartements ne se touchent pas et ne touchent pas la cloison qui les entoure : la double page laisse un écart de tous les côtés, et cet écart se parcourt.
  - Chap. 363 — la double page dessine un couloir au dos des appartements, entre les deux rangées : les quatorze lui présentent leur dos, et aucun n’y a de porte.
- **Sources du mobilier et des détails :**
  - Chap. 367 à 369 — le réseau de service du secteur d'habitation : le cafard de Bill le parcourt de suite en suite, le chap. 369 en dessine l'intérieur et la feuille du quartier qui lui sert de carte, et le chap. 368 regarde la chambre du prince Momoze par la grille où il aboutit.

## Chambre 1001 (Benjamin) — `tier-1-royal-residential-sector-room-1001`

- **Pont :** tier-1, interior-room-1001
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 9
- **Emprises relevées :** 17 × 5 m ; 9 × 7.5 m ; 2.4 × 2.1 m ; 3 × 7.5 m ; 9 × 4.5 m ; 9 × 6 m ; 21 × 8.4 m ; 15 × 4.5 m ; 6 × 4.5 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 4 cabinet, 3 basin, 3 bed, 2 counter, 2 table, 1 painting, 1 seat, 1 telephone
- **Objets directement visibles dans les cases :** 17/17
- **Espaces internes :** Quartiers des domestiques — 1001 (Benjamin) ; WC du personnel — 1001 (Benjamin) ; Vestibule d'entrée — 1001 (Benjamin) ; Cuisine — 1001 (Benjamin) ; Salle à manger — 1001 (Benjamin) ; Salon — 1001 (Benjamin) ; Chambre du prince — 1001 (Benjamin) ; Bain et WC — 1001 (Benjamin)
- **Sources de la pièce :**
  - Chap. 363 — la double page de la zone d'habitation : quatorze appartements isolés en deux rangées de sept à l’intérieur de la cloison intérieure, impairs à tribord, chacun avec son unique porte face à la porte numérotée qui lui est assignée ; les numéros de chambre viennent du chap. 349.
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
- **Sources du mobilier et des détails :**
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
  - Chap. 366 — le passage dans la chambre 1001 montre Balsamilco au téléphone près de la porte de Benjamin, tandis que Furykov se tient dans le salon de réception.
  - Chap. 363 — le discours de Benjamin à ses soldats est cadré par le grand tableau accroché dans son salon de réception.

## Chambre 1002 (Camilla) — `tier-1-royal-residential-sector-room-1002`

- **Pont :** tier-1, interior-room-1002
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 9
- **Emprises relevées :** 17 × 5 m ; 9 × 7.5 m ; 2.4 × 2.1 m ; 3 × 7.5 m ; 9 × 4.5 m ; 9 × 6 m ; 21 × 8.4 m ; 15 × 4.5 m ; 6 × 4.5 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 4 cabinet, 4 seat, 3 basin, 3 bed, 3 table, 2 counter
- **Objets directement visibles dans les cases :** 19/19
- **Espaces internes :** Quartiers des domestiques — 1002 (Camilla) ; WC du personnel — 1002 (Camilla) ; Vestibule d'entrée — 1002 (Camilla) ; Cuisine — 1002 (Camilla) ; Salle à manger — 1002 (Camilla) ; Salon — 1002 (Camilla) ; Chambre du prince — 1002 (Camilla) ; Bain et WC — 1002 (Camilla)
- **Sources de la pièce :**
  - Chap. 363 — la double page de la zone d'habitation : quatorze appartements isolés en deux rangées de sept à l’intérieur de la cloison intérieure, impairs à tribord, chacun avec son unique porte face à la porte numérotée qui lui est assignée ; les numéros de chambre viennent du chap. 349.
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
- **Sources du mobilier et des détails :**
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
  - Chap. 389 — les gardes Intouchables de Camilla se réunissent dans un salon formel tapissé, assis le long des murs autour d'un vaste centre dégagé.
  - Chap. 366 — Camilla mange avec la reine Duazul tandis que Musse occupe un bureau séparé dans le passage de la chambre 1002.

## Chambre 1003 (Zhang Lei) — `tier-1-royal-residential-sector-room-1003`

- **Pont :** tier-1, interior-room-1003
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 9
- **Emprises relevées :** 17 × 5 m ; 9 × 7.5 m ; 2.4 × 2.1 m ; 3 × 7.5 m ; 9 × 4.5 m ; 9 × 6 m ; 21 × 8.4 m ; 15 × 4.5 m ; 6 × 4.5 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 7 cabinet, 3 basin, 3 bed, 3 seat, 3 table, 2 counter, 1 painting
- **Objets directement visibles dans les cases :** 22/22
- **Espaces internes :** Quartiers des domestiques — 1003 (Zhang Lei) ; WC du personnel — 1003 (Zhang Lei) ; Vestibule d'entrée — 1003 (Zhang Lei) ; Cuisine — 1003 (Zhang Lei) ; Salle à manger — 1003 (Zhang Lei) ; Salon — 1003 (Zhang Lei) ; Chambre du prince — 1003 (Zhang Lei) ; Bain et WC — 1003 (Zhang Lei)
- **Sources de la pièce :**
  - Chap. 363 — la double page de la zone d'habitation : quatorze appartements isolés en deux rangées de sept à l’intérieur de la cloison intérieure, impairs à tribord, chacun avec son unique porte face à la porte numérotée qui lui est assignée ; les numéros de chambre viennent du chap. 349.
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
- **Sources du mobilier et des détails :**
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
  - Chap. 365 — la salle à manger du prince Zhang Lei, où il reçoit la reine Oito : la longue table sculptée, un fauteuil à haut dossier à chacun de ses bouts, la crédence et la statue sur sa sellette le long du mur tribord, le cadre au mur et le guéridon près de la porte.
  - Chap. 366 — le passage dans la chambre 1003 isole le haut candélabre à branches placé près de la table d'apparat de Zhang Lei.

## Chambre 1004 (Tserriednich) — `tier-1-royal-residential-sector-room-1004`

- **Pont :** tier-1, interior-room-1004
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 9
- **Emprises relevées :** 17 × 5 m ; 9 × 7.5 m ; 2.4 × 2.1 m ; 3 × 7.5 m ; 9 × 4.5 m ; 9 × 6 m ; 21 × 8.4 m ; 15 × 4.5 m ; 6 × 4.5 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 5 cabinet, 3 basin, 3 bed, 2 counter, 2 table, 1 painting, 1 seat
- **Objets directement visibles dans les cases :** 17/17
- **Espaces internes :** Quartiers des domestiques — 1004 (Tserriednich) ; WC du personnel — 1004 (Tserriednich) ; Vestibule d'entrée — 1004 (Tserriednich) ; Cuisine — 1004 (Tserriednich) ; Salle à manger — 1004 (Tserriednich) ; Salon — 1004 (Tserriednich) ; Chambre du prince — 1004 (Tserriednich) ; Bain et WC — 1004 (Tserriednich)
- **Sources de la pièce :**
  - Chap. 363 — la double page de la zone d'habitation : quatorze appartements isolés en deux rangées de sept à l’intérieur de la cloison intérieure, impairs à tribord, chacun avec son unique porte face à la porte numérotée qui lui est assignée ; les numéros de chambre viennent du chap. 349.
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
- **Sources du mobilier et des détails :**
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
  - Chap. 384–387 — le salon presque vide où Tserriednich s'entraîne au Nen présente un sol géométrique poli, de hautes portes à panneaux, des murs moulurés, un cadre et une pendule sur le manteau.

## Chambre 1005 (Tubeppa) — `tier-1-royal-residential-sector-room-1005`

- **Pont :** tier-1, interior-room-1005
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 9
- **Emprises relevées :** 17 × 5 m ; 9 × 7.5 m ; 2.4 × 2.1 m ; 3 × 7.5 m ; 9 × 4.5 m ; 9 × 6 m ; 21 × 8.4 m ; 15 × 4.5 m ; 6 × 4.5 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 4 cabinet, 3 basin, 3 bed, 3 painting, 3 table, 2 counter, 2 seat
- **Objets directement visibles dans les cases :** 20/20
- **Espaces internes :** Quartiers des domestiques — 1005 (Tubeppa) ; WC du personnel — 1005 (Tubeppa) ; Vestibule d'entrée — 1005 (Tubeppa) ; Cuisine — 1005 (Tubeppa) ; Salle à manger — 1005 (Tubeppa) ; Salon — 1005 (Tubeppa) ; Chambre du prince — 1005 (Tubeppa) ; Bain et WC — 1005 (Tubeppa)
- **Sources de la pièce :**
  - Chap. 363 — la double page de la zone d'habitation : quatorze appartements isolés en deux rangées de sept à l’intérieur de la cloison intérieure, impairs à tribord, chacun avec son unique porte face à la porte numérotée qui lui est assignée ; les numéros de chambre viennent du chap. 349.
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
- **Sources du mobilier et des détails :**
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
  - Chap. 366 — les appartements de la princesse Tubeppa rendus à son travail : le bureau où elle s'assied devant son ordinateur, son siège, les tableaux blancs couverts d'équations le long du mur tribord, et le panneau de feuilles épinglées à côté d'eux.

## Chambre 1006 (Tyson) — `tier-1-royal-residential-sector-room-1006`

- **Pont :** tier-1, interior-room-1006
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 9
- **Emprises relevées :** 17 × 5 m ; 9 × 7.5 m ; 2.4 × 2.1 m ; 3 × 7.5 m ; 9 × 4.5 m ; 9 × 6 m ; 21 × 8.4 m ; 15 × 4.5 m ; 6 × 4.5 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 6 cabinet, 3 basin, 3 bed, 3 table, 2 counter, 1 seat
- **Objets directement visibles dans les cases :** 18/18
- **Espaces internes :** Quartiers des domestiques — 1006 (Tyson) ; WC du personnel — 1006 (Tyson) ; Vestibule d'entrée — 1006 (Tyson) ; Cuisine — 1006 (Tyson) ; Salle à manger — 1006 (Tyson) ; Salon — 1006 (Tyson) ; Chambre du prince — 1006 (Tyson) ; Bain et WC — 1006 (Tyson)
- **Sources de la pièce :**
  - Chap. 363 — la double page de la zone d'habitation : quatorze appartements isolés en deux rangées de sept à l’intérieur de la cloison intérieure, impairs à tribord, chacun avec son unique porte face à la porte numérotée qui lui est assignée ; les numéros de chambre viennent du chap. 349.
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
- **Sources du mobilier et des détails :**
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
  - Chap. 389 — Tyson entre par de hautes portes à panneaux dans une réception d'anniversaire ornée ; un gâteau et des cadeaux occupent la table centrale tandis que les assistants longent les murs.
  - Chap. 389 — la réception d'anniversaire de Tyson est garnie de cadeaux emballés autour de son présentoir central.

## Chambre 1007 (Luzurus) — `tier-1-royal-residential-sector-room-1007`

- **Pont :** tier-1, interior-room-1007
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 9
- **Emprises relevées :** 17 × 5 m ; 9 × 7.5 m ; 2.4 × 2.1 m ; 3 × 7.5 m ; 9 × 4.5 m ; 9 × 6 m ; 21 × 8.4 m ; 15 × 4.5 m ; 6 × 4.5 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 7 cabinet, 4 painting, 3 basin, 3 bed, 3 table, 2 counter, 2 seat
- **Objets directement visibles dans les cases :** 24/24
- **Espaces internes :** Quartiers des domestiques — 1007 (Luzurus) ; WC du personnel — 1007 (Luzurus) ; Vestibule d'entrée — 1007 (Luzurus) ; Cuisine — 1007 (Luzurus) ; Salle à manger — 1007 (Luzurus) ; Salon — 1007 (Luzurus) ; Chambre du prince — 1007 (Luzurus) ; Bain et WC — 1007 (Luzurus)
- **Sources de la pièce :**
  - Chap. 363 — la double page de la zone d'habitation : quatorze appartements isolés en deux rangées de sept à l’intérieur de la cloison intérieure, impairs à tribord, chacun avec son unique porte face à la porte numérotée qui lui est assignée ; les numéros de chambre viennent du chap. 349.
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
- **Sources du mobilier et des détails :**
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
  - Chap. 386 et 389 — le salon habité de Luzurus est encombré de sièges profondément rembourrés, de tables d'appoint, de bouteilles, de plantes, d'étagères et d'ornements.
  - Chap. 414 — Luzurus regarde un film dans la chambre 1007 avec Rice et Basho ; l'écran appartient au salon encombré montré auparavant.
  - Chap. 386 — la vue d'ensemble du salon de Luzurus montre deux topiaires ronds assortis encadrant les sièges rembourrés.
  - Chap. 386 et 389 — le salon encombré de Luzurus est bordé de grands tableaux encadrés au-dessus et entre les sièges et les meubles.

## Chambre 1008 (Salé-salé) — `tier-1-royal-residential-sector-room-1008`

- **Pont :** tier-1, interior-room-1008
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 9
- **Emprises relevées :** 17 × 5 m ; 9 × 7.5 m ; 2.4 × 2.1 m ; 3 × 7.5 m ; 9 × 4.5 m ; 9 × 6 m ; 21 × 8.4 m ; 15 × 4.5 m ; 6 × 4.5 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 5 cabinet, 3 basin, 3 bed, 3 table, 2 counter, 1 painting, 1 seat
- **Objets directement visibles dans les cases :** 18/18
- **Espaces internes :** Quartiers des domestiques — 1008 (Salé-salé) ; WC du personnel — 1008 (Salé-salé) ; Vestibule d'entrée — 1008 (Salé-salé) ; Cuisine — 1008 (Salé-salé) ; Salle à manger — 1008 (Salé-salé) ; Salon — 1008 (Salé-salé) ; Chambre du prince — 1008 (Salé-salé) ; Bain et WC — 1008 (Salé-salé)
- **Sources de la pièce :**
  - Chap. 363 — la double page de la zone d'habitation : quatorze appartements isolés en deux rangées de sept à l’intérieur de la cloison intérieure, impairs à tribord, chacun avec son unique porte face à la porte numérotée qui lui est assignée ; les numéros de chambre viennent du chap. 349.
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
- **Sources du mobilier et des détails :**
  - Chap. 366 et 382 — Salé-salé reste couché avec deux compagnes dans le large lit qui transforme sa chambre en pièce de fête informelle ; Predator et Stand By Me l'atteignent tous deux à côté de ce lit.
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
  - Chap. 366 et 382 — la chambre au large lit de Salé-salé sert aussi de pièce de fête informelle, avec ses compagnes et les boissons gardées près du lit.
  - Chap. 362 et 382 — la chambre de Salé-salé sert de salle de fête : un ensemble audiovisuel et des enceintes fait face au grand lit, parmi nourriture, bouteilles et coussins.

## Chambre 1009 (Halkenburg) — `tier-1-royal-residential-sector-room-1009`

- **Pont :** tier-1, interior-room-1009
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 9
- **Emprises relevées :** 17 × 5 m ; 9 × 7.5 m ; 2.4 × 2.1 m ; 3 × 7.5 m ; 9 × 4.5 m ; 9 × 6 m ; 21 × 8.4 m ; 15 × 4.5 m ; 6 × 4.5 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 7 cabinet, 7 seat, 3 basin, 3 bed, 2 counter, 2 table
- **Objets directement visibles dans les cases :** 24/24
- **Espaces internes :** Quartiers des domestiques — 1009 (Halkenburg) ; WC du personnel — 1009 (Halkenburg) ; Vestibule d'entrée — 1009 (Halkenburg) ; Cuisine — 1009 (Halkenburg) ; Salle à manger — 1009 (Halkenburg) ; Salon — 1009 (Halkenburg) ; Chambre du prince — 1009 (Halkenburg) ; Bain et WC — 1009 (Halkenburg)
- **Sources de la pièce :**
  - Chap. 363 — la double page de la zone d'habitation : quatorze appartements isolés en deux rangées de sept à l’intérieur de la cloison intérieure, impairs à tribord, chacun avec son unique porte face à la porte numérotée qui lui est assignée ; les numéros de chambre viennent du chap. 349.
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
- **Sources du mobilier et des détails :**
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
  - Chap. 382 et 386 — le salon de Halkenburg laisse un vaste centre carrelé dégagé, les chaises étant disposées en cercle autour du corps de Shikaku occupé.
  - Chap. 386 — les vues larges du salon de Halkenburg montrent des bibliothèques et vitrines longeant le mur autour du manteau central, derrière le cercle de chaises.

## Chambre 1010 (Kacho) — `tier-1-royal-residential-sector-room-1010`

- **Pont :** tier-1, interior-room-1010
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 9
- **Emprises relevées :** 17 × 5 m ; 9 × 7.5 m ; 2.4 × 2.1 m ; 3 × 7.5 m ; 9 × 4.5 m ; 9 × 6 m ; 21 × 8.4 m ; 15 × 4.5 m ; 6 × 4.5 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 6 cabinet, 3 basin, 3 bed, 3 counter, 2 painting, 2 seat, 2 table
- **Objets directement visibles dans les cases :** 21/21
- **Espaces internes :** Quartiers des domestiques — 1010 (Kacho) ; WC du personnel — 1010 (Kacho) ; Vestibule d'entrée — 1010 (Kacho) ; Cuisine — 1010 (Kacho) ; Salle à manger — 1010 (Kacho) ; Salon — 1010 (Kacho) ; Chambre du prince — 1010 (Kacho) ; Bain et WC — 1010 (Kacho)
- **Sources de la pièce :**
  - Chap. 363 — la double page de la zone d'habitation : quatorze appartements isolés en deux rangées de sept à l’intérieur de la cloison intérieure, impairs à tribord, chacun avec son unique porte face à la porte numérotée qui lui est assignée ; les numéros de chambre viennent du chap. 349.
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362. Le chap. 376 s'y tient : le plan de travail sous ses éléments hauts, la plaque et son four encastré, le retour de plan le long du mur tribord, et les placards bas que le morse de Senritsu dénombre.
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362. Le chap. 376 le montre : Kacho sur ses devoirs sous le plafond à caissons, le mur lambrissé derrière elle et ses tableaux encadrés, et dehors la porte du 1010.
- **Sources du mobilier et des détails :**
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
  - Chap. 376 — la cuisine de la princesse Kacho, dessinée deux fois : le plan de travail sous ses éléments hauts, la plaque de cuisson avec sa casserole et le four encastré dessous, le retour de plan le long du mur tribord, et la rangée de placards bas sous les deux. Le morse de Senritsu en désigne un exactement — cuisine, placard de gauche, en bas, au fond.
  - Chap. 376 — le salon de la princesse Kacho, où on la fait asseoir sur ses devoirs : le mur lambrissé derrière elle et ses tableaux encadrés, sous le plafond à caissons commun aux appartements.
  - Chap. 374 — Kacho est contrainte de travailler dans un salon d'apparat meublé d'une commode basse sous les tableaux et d'un petit tabouret rond placé à côté.

## Chambre 1011 (Fugetsu) — `tier-1-royal-residential-sector-room-1011`

- **Pont :** tier-1, interior-room-1011
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 9
- **Emprises relevées :** 17 × 5 m ; 9 × 7.5 m ; 2.4 × 2.1 m ; 3 × 7.5 m ; 9 × 4.5 m ; 9 × 6 m ; 21 × 8.4 m ; 15 × 4.5 m ; 6 × 4.5 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 6 cabinet, 3 basin, 3 bed, 2 counter, 2 table, 1 seat
- **Objets directement visibles dans les cases :** 17/17
- **Espaces internes :** Quartiers des domestiques — 1011 (Fugetsu) ; WC du personnel — 1011 (Fugetsu) ; Vestibule d'entrée — 1011 (Fugetsu) ; Cuisine — 1011 (Fugetsu) ; Salle à manger — 1011 (Fugetsu) ; Salon — 1011 (Fugetsu) ; Chambre du prince — 1011 (Fugetsu) ; Bain et WC — 1011 (Fugetsu)
- **Sources de la pièce :**
  - Chap. 363 — la double page de la zone d'habitation : quatorze appartements isolés en deux rangées de sept à l’intérieur de la cloison intérieure, impairs à tribord, chacun avec son unique porte face à la porte numérotée qui lui est assignée ; les numéros de chambre viennent du chap. 349.
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
- **Sources du mobilier et des détails :**
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
  - Chap. 366 et 374 — Fugetsu regarde des photographies dans son lit ; lorsque Magical Worm s'ouvre, le réveil numérique à côté d'elle indique mardi, 01 h 27.
  - Chap. 366 et 374 — le grand lit de Fugetsu est entouré de livres et de photographies éparses ; elle étale les clichés sur la couverture avant l'ouverture de Magical Worm.

## Chambre 1012 (Momoze) — `tier-1-royal-residential-sector-room-1012`

- **Pont :** tier-1, interior-room-1012
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 9
- **Emprises relevées :** 17 × 5 m ; 9 × 7.5 m ; 2.4 × 2.1 m ; 3 × 7.5 m ; 9 × 4.5 m ; 9 × 6 m ; 21 × 8.4 m ; 15 × 4.5 m ; 6 × 4.5 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 5 cabinet, 3 basin, 3 bed, 2 counter, 2 table, 1 seat, 1 telephone, 1 vent
- **Objets directement visibles dans les cases :** 18/18
- **Espaces internes :** Quartiers des domestiques — 1012 (Momoze) ; WC du personnel — 1012 (Momoze) ; Vestibule d'entrée — 1012 (Momoze) ; Cuisine — 1012 (Momoze) ; Salle à manger — 1012 (Momoze) ; Salon — 1012 (Momoze) ; Chambre du prince — 1012 (Momoze) ; Bain et WC — 1012 (Momoze)
- **Sources de la pièce :**
  - Chap. 363 — la double page de la zone d'habitation : quatorze appartements isolés en deux rangées de sept à l’intérieur de la cloison intérieure, impairs à tribord, chacun avec son unique porte face à la porte numérotée qui lui est assignée ; les numéros de chambre viennent du chap. 349.
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
- **Sources du mobilier et des détails :**
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
  - Chap. 367 — le même réseau, un appartement plus loin : le cafard sort des égouts par la grille de la grande chambre du prince Momoze et surplombe le lit. C'est par cette grille que la reine Oito assiste au meurtre de Momoze au chap. 368.
  - Chap. 373 — les gardes armés refusent l'accès à la chambre 1012 scellée, même lorsqu'un visiteur demande seulement à utiliser le téléphone qui se trouve à l'intérieur.
  - Chap. 366 à 368 — Momoze dort dans un grand lit à rideaux, hauts montants et baldaquin ; la grille de service domine cette tête de lit caractéristique.

## Chambre 1013 (Marayam) — `tier-1-royal-residential-sector-room-1013`

- **Pont :** tier-1, interior-room-1013
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 9
- **Emprises relevées :** 17 × 5 m ; 9 × 7.5 m ; 2.4 × 2.1 m ; 3 × 7.5 m ; 9 × 4.5 m ; 9 × 6 m ; 21 × 8.4 m ; 15 × 4.5 m ; 6 × 4.5 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 5 cabinet, 3 basin, 3 bed, 3 table, 2 counter, 1 painting, 1 seat, 1 vent
- **Objets directement visibles dans les cases :** 19/19
- **Espaces internes :** Quartiers des domestiques — 1013 (Marayam) ; WC du personnel — 1013 (Marayam) ; Vestibule d'entrée — 1013 (Marayam) ; Cuisine — 1013 (Marayam) ; Salle à manger — 1013 (Marayam) ; Salon — 1013 (Marayam) ; Chambre du prince — 1013 (Marayam) ; Bain et WC — 1013 (Marayam)
- **Sources de la pièce :**
  - Chap. 363 — la double page de la zone d'habitation : quatorze appartements isolés en deux rangées de sept à l’intérieur de la cloison intérieure, impairs à tribord, chacun avec son unique porte face à la porte numérotée qui lui est assignée ; les numéros de chambre viennent du chap. 349.
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
- **Sources du mobilier et des détails :**
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
  - Chap. 367 — le réseau d'égouts sous la zone d'habitation et les grilles à persiennes par lesquelles il débouche dans les appartements : le cafard de Bill est lâché dans les égouts et gagne ainsi un prince après l'autre. Les pièces du prince Marayam sont les premières qu'il observe d'en haut, juste sous le plafond.
  - Chap. 366 — la chambre protégée de Marayam contient le mobilier de jeu bas où sont gardées ses voitures miniatures et ses briques, tandis que domestiques et gardes veillent sur lui.
  - Chap. 366 — des voitures miniatures et des briques de construction identifient la chambre protégée de Marayam comme l'espace de vie d'un enfant.
  - Chap. 366 — le tableau encadré derrière la reine Sevanti et Marayam appartient au salon meublé montré avant que la pièce ne soit isolée par le Nen.

## Chambre 1014 (Woble) — `tier-1-royal-residential-sector-room-1014`

- **Pont :** tier-1, interior-room-1014
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 9
- **Emprises relevées :** 17 × 5 m ; 9 × 7.5 m ; 2.4 × 2.1 m ; 3 × 7.5 m ; 9 × 4.5 m ; 9 × 6 m ; 21 × 8.4 m ; 15 × 4.5 m ; 6 × 4.5 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 8 cabinet, 4 table, 3 basin, 3 bed, 3 painting, 2 counter, 2 seat, 1 telephone
- **Objets directement visibles dans les cases :** 26/26
- **Espaces internes :** Quartiers des domestiques — 1014 (Woble) ; WC du personnel — 1014 (Woble) ; Vestibule d'entrée — 1014 (Woble) ; Cuisine — 1014 (Woble) ; Salle à manger — 1014 (Woble) ; Salon — 1014 (Woble) ; Chambre du prince — 1014 (Woble) ; Bain et WC — 1014 (Woble)
- **Sources de la pièce :**
  - Chap. 363 — la double page de la zone d'habitation : quatorze appartements isolés en deux rangées de sept à l’intérieur de la cloison intérieure, impairs à tribord, chacun avec son unique porte face à la porte numérotée qui lui est assignée ; les numéros de chambre viennent du chap. 349.
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362. Le chap. 367 se tient dedans : le plan de travail contre la cloison avant sous ses éléments hauts vitrés, et un plancher là où le reste de l'appartement est dessiné carrelé.
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362. Le chap. 371 s'y tient : deux longues tables couvertes de plateaux-repas sous un plafond à caissons, les murs lambrissés du côté public de l'appartement, et au bout la porte de la cuisine.
- **Sources du mobilier et des détails :**
  - Chap. 363 — le plan d'un appartement princier, encarté sur la double page de la zone d'habitation : salle du personnel, cuisine, salle à manger, salon, chambre du prince et salle d'eau derrière une seule porte, et les pièces montrées pour Benjamin, Tserriednich et Luzurus à partir du chap. 362.
  - Chap. 371 — la salle à manger dressée pour la maison, où Kurapika arrête une servante entre les tables et lui pose ses deux questions : deux longues tables la traversent sous un plafond à caissons, couvertes de plateaux-repas, chaises à haut dossier le long de leurs côtés, et entre elles une allée où l'on sert. Le plan du chap. 363 n'y met qu'une petite table ; la planche, elle, montre la pièce.
  - Chap. 360 — les appartements du prince Woble tels que Kurapika les trouve : le téléphone mural par lequel il appelle Biscuit, l'armoire et la table de présentation le long du mur tribord, le fauteuil canné, les deux sellettes à sculpture, et les tableaux encadrés le long du mur lambrissé.
  - Chap. 367 — la cuisine du prince Woble vue depuis le sol : le plan de travail contre la cloison avant, les éléments hauts vitrés portés au-dessus, les bouteilles posées sur le plan, et le plancher où attendent les gardes du corps.

## Quartiers des soldats — `tier-1-soldiers-living-quarters`

- **Pont :** tier-1-b, interior-soldiers-quarters
- **Preuve de l’enveloppe :** plan publié
- **Sous-espaces :** 10
- **Emprises relevées :** 33.25 × 52.5 m ; 45 × 5.5 m ; 10 × 7.5 m
- **Hauteurs sous plafond :** 3.5 m
- **Éléments modélisés :** 96 bed
- **Objets directement visibles dans les cases :** 0/96
- **Espaces internes :** Passage gardé vers les ponts inférieurs ; Caserne de l'armée royale 1 ; Caserne de l'armée royale 2 ; Caserne de l'armée royale 3 ; Caserne de l'armée royale 4 ; Quartiers des Hunters provisoires 1 ; Quartiers des Hunters provisoires 2 ; Quartiers des Hunters provisoires 3 ; Quartiers des Hunters provisoires 4
- **Sources de la pièce :**
  - Plan des ponts, chap. 349
  - Le plan dessine les pièces mais aucun passage entre elles ; cette circulation est reconstruite. Le plan dessine le passage plus étroit que les salles qu’il dessert.
  - Plan des quartiers des soldats ; soldats de Kakin et quelque 150 Hunters provisoires
- **Sources du mobilier et des détails :**
  - Le plan des quartiers des soldats sur /ship dessine douze couchettes dans chaque chambrée et dans chaque salle des Hunters ; la reconstruction en garde le nombre et les rangées, et donne aux allées de quoi passer.

## Cour suprême — `tier-1-supreme-court`

- **Pont :** tier-1-b
- **Preuve de l’enveloppe :** plan publié
- **Sous-espaces :** 1
- **Emprises relevées :** 42 × 28 m
- **Hauteurs sous plafond :** 8 m
- **Éléments modélisés :** 2 counter, 2 table, 1 painting, 1 seat
- **Objets directement visibles dans les cases :** 2/6
- **Sources de la pièce :**
  - Plan des ponts, chap. 349
- **Sources du mobilier et des détails :**
  - Chap. 371 — Cleapatro reçoit la requête d’urgence dans la salle des juges de la Cour suprême, assis derrière son haut banc ornementé.
  - Chap. 371 — un grand emblème circulaire de la Cour est centré sur le mur à panneaux au-dessus du banc de Cleapatro.
  - Le plan de salle d'audience de /ship : le banc du magistrat en tête, l'accusation et la défense face à face, le box entre les deux et le banc du public au fond.

## Casino VIP — `tier-1-vip-casino`

- **Pont :** tier-1-c
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 1
- **Emprises relevées :** 17.5 × 52.5 m
- **Hauteurs sous plafond :** 8 m
- **Éléments modélisés :** 19 table, 14 counter, 4 seat, 2 platform
- **Objets directement visibles dans les cases :** 39/39
- **Sources de la pièce :**
  - Chap. 405 — la salle de jeu, les boutiques autour et la mezzanine au-dessus.
- **Sources du mobilier et des détails :**
  - Chap. 405 — la salle de jeu, les boutiques autour et la mezzanine au-dessus.
  - Chap. 405 — la salle de jeu, les devantures autour et la mezzanine au-dessus : les boutiques courent sur toute la longueur des deux murs à chaque niveau, et un salon de fauteuils occupe le milieu de la salle.

## Prison VIP — `tier-1-vip-jail`

- **Pont :** tier-1-b, interior-vip-detention
- **Preuve de l’enveloppe :** case du manga, plan publié
- **Sous-espaces :** 6
- **Emprises relevées :** 31.5 × 14 m ; 20 × 2.2 m ; 5.5 × 4.5 m ; 3.3 × 4.5 m
- **Hauteurs sous plafond :** 3.5 m
- **Éléments modélisés :** 8 bars, 4 basin, 2 bed, 1 counter
- **Objets directement visibles dans les cases :** 7/15
- **Espaces internes :** Coursive du quartier ; Cellule première classe ; Cellule VIP vide ; Cellule standard ; Cellule
- **Sources de la pièce :**
  - Plan des ponts, chap. 349
  - Chap. 372 — le couloir du quartier et le gardien à son bureau, sur le chemin des cellules des gardes détenus.
  - Chap. 373 — la cellule de première classe vue de dessus : la couchette contre le mur latéral, la cuvette sur le mur d'en face, le lavabo sous le mur du fond, et toute la façade en barreaux. C'est la couchette qui mesure la pièce.
  - Plan du quartier de détention VIP ; le bloc montré au chap. 350
  - Chap. 372 — la cellule où est détenu l'un des gardes de Momoze : couchette, lavabo et cabinet, vus de dessus.
- **Sources du mobilier et des détails :**
  - Chap. 373 — la cellule de première classe vue de dessus : la couchette contre le mur latéral, la cuvette sur le mur d'en face, le lavabo sous le mur du fond, et toute la façade en barreaux. C'est la couchette qui mesure la pièce.
  - Chap. 372 — la cellule où est détenu l'un des gardes de Momoze : couchette, lavabo et cabinet, vus de dessus.
  - Chap. 372 — le couloir du quartier et le gardien à son bureau, sur le chemin des cellules des gardes détenus.
  - Plan du quartier de détention VIP ; le bloc montré au chap. 350

## Couloir du bloc VVIP — `tier-1-vvip-living-quarters`

- **Pont :** tier-1
- **Preuve de l’enveloppe :** reconstruction cartographique
- **Sous-espaces :** 1
- **Emprises relevées :** 3 × 52.5 m
- **Éléments modélisés :** aucun objet non attesté
- **Objets directement visibles dans les cases :** 0/0
- **Sources de la pièce :**
  - Le plan du bloc sur /ship : un couloir dans sa longueur, quatre suites de chaque côté.
- **Limite connue :** aucune case ne justifie davantage de mobilier dans ce volume.

## Cellule de Beyond Netero — `tier-1-vvip-prison-beyond`

- **Pont :** tier-1-b, interior-beyond-cell
- **Preuve de l’enveloppe :** case du manga, plan publié
- **Sous-espaces :** 3
- **Emprises relevées :** 31.5 × 14 m ; 8.75 × 10 m ; 6.25 × 10 m
- **Hauteurs sous plafond :** 3.5 m
- **Éléments modélisés :** 3 counter, 2 bars, 2 table, 1 basin, 1 bed, 1 cabinet, 1 manacle, 1 seat, 1 telephone
- **Objets directement visibles dans les cases :** 9/13
- **Espaces internes :** Poste de garde des Zodiaques ; La cellule même
- **Sources de la pièce :**
  - Plan des ponts, chap. 349
  - Plan de la cellule de haute sécurité ; la garde des Zodiaques tenue devant les barreaux, chap. 359
  - Le chap. 350 montre la cellule de haute sécurité et ses entraves ; le chap. 359, la façade à barreaux et la garde tenue devant.
- **Sources du mobilier et des détails :**
  - Le chap. 350 montre la cellule de haute sécurité et ses entraves ; le chap. 359, la façade à barreaux et la garde tenue devant.
  - Plan de la cellule de haute sécurité ; la garde des Zodiaques tenue devant les barreaux, chap. 359
  - Chap. 359 — le bloc cellulaire du niveau 1 : dans la cellule, la table basse sur laquelle repose la boisson de Beyond, et hors des barreaux le banc de la garde, sa table basse, l'étagère contre le mur arrière et le poste mural près de la porte.

## Cloison blindée du pont 2 — `tier-2-bulkhead`

- **Pont :** tier-2
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 1
- **Emprises relevées :** 245 × 12.25 m
- **Éléments modélisés :** 2 cabinet, 1 counter
- **Objets directement visibles dans les cases :** 0/3
- **Sources de la pièce :**
  - Chap. 349 — cloison blindée, 600 gardes en poste
- **Sources du mobilier et des détails :**
  - Le plan de la cloison sur /ship : la porte blindée et ses verrous, et le panneau d'ouverture d'urgence du côté du pont 2 — le seul d'où elle s'ouvre.

## Planque secrète Heil-Ly — `tier-2-heilly-secret-hideout`

- **Pont :** tier-2, interior-heilly-hideout
- **Preuve de l’enveloppe :** case du manga, plan publié, liaison déduite
- **Sous-espaces :** 6
- **Emprises relevées :** 26.25 × 52.5 m ; 10.52 × 5.25 m ; 33.63 × 6.3 m ; 10.52 × 8.05 m ; 10.51 × 8.05 m ; 6.3 × 8.05 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 12 seat, 10 cabinet, 5 table, 3 basin, 3 vent, 1 duct
- **Objets directement visibles dans les cases :** 29/34
- **Espaces internes :** Salle de traitement / douches ; Couloir de bunker ; Buanderie / élimination ; Salle commune ; Bureau du chef
- **Sources de la pièce :**
  - Chap. 356 — dissimulée dans le bloc VIP ; la pièce exacte n'est pas montrée.
  - Chap. 398, p. 158-167 — grande salle de traitement carrelée, protégée par une barrière de Nen dans les murs, avec sept portes, flexibles de douche, siphons de sol, petits luminaires muraux et placard de maintenance ouvert ; les portes desservent trois WC, un bain, une cabine de douche, la maintenance et la voie du portail.
  - Reconstruit pour que les pièces dessinées communiquent ; un couloir de bunker est dessiné, mais pas son tracé.
  - Chap. 399, p. 168-169 — la buanderie / salle d'élimination : batteries de lave-linge et sèche-linge industriels frontaux, linge suspendu, étagères murales chargées de cartons et fournitures pliées, éviers, casiers hauts, flexibles techniques, siphons de sol et plusieurs tables métalliques centrales.
  - Chap. 399, p. 170-175 — la salle commune : longue table centrale entourée d'au moins huit chaises, tuyaux et chemins de câbles apparents au plafond, étagères et casiers muraux, poubelles, fontaine à eau, petits appareils, horloge et avis encadrés.
  - Plan de la planque Heil-Ly ; les sept portes de la salle de traitement, la buanderie, la salle commune et le bureau du chef
- **Sources du mobilier et des détails :**
  - Chap. 398, p. 158-167 — la salle de traitement carrelée montre des flexibles de douche et siphons dans l'espace ouvert, un placard de maintenance ouvert avec étagères, et une barrière de Nen parcourant ses murs.
  - Chap. 399, p. 168-169 — la buanderie / salle d'élimination contient visiblement des machines industrielles frontales, éviers, rangements muraux, casiers, siphons, flexibles et au moins deux tables métalliques centrales.
  - Chap. 399, p. 170-175 — la salle commune contient visiblement la longue table et au moins huit chaises, des rangements muraux, une fontaine à eau et les réseaux apparents du plafond.
  - Plan de la planque Heil-Ly ; les sept portes de la salle de traitement, la buanderie, la salle commune et le bureau du chef
  - Chap. 407 — le grand téléviseur sombre et son meuble bas occupent le mur derrière la chaise de bureau de Morena.
  - Chap. 399, p. 3 — le flanc d’un canapé est visible contre le mur du bureau lorsque Hinrigh ouvre la porte de la planque ; le chap. 407 le montre en face de la table de Morena.
  - Chap. 407 — une Famicom repose sur le meuble du téléviseur derrière Morena ; sa coque claire se détache de l’écran sombre.
  - Chap. 407 — une Super Famicom est posée à côté de la Famicom sur le meuble du téléviseur derrière Morena.
  - Chap. 407-410 — la table sur laquelle Morena Prudo et Borksen jouent la partie de négociation, et une chaise de chaque côté, dans le bureau du chef que dessine le plan de la planque.
- **Limite connue :** 1 liaison(s) ou volume(s) restent déduits pour assurer la continuité de la visite.

## Ministère de la Justice — `tier-2-ministry-of-justice`

- **Pont :** tier-2, interior-justice-bureau
- **Preuve de l’enveloppe :** case du manga, plan publié, liaison déduite
- **Sous-espaces :** 6
- **Emprises relevées :** 63 × 45.5 m ; 12.5 × 9.5 m ; 13.5 × 9.5 m ; 20.25 × 13.5 m ; 41.5 × 1.75 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 3 table, 2 cabinet, 2 counter, 2 seat
- **Objets directement visibles dans les cases :** 5/9
- **Espaces internes :** Hall d’accueil ; Bureau du vice-ministre ; Salles d'interrogatoire ; Parloir ; Couloir du bureau
- **Sources de la pièce :**
  - Plan des ponts, chap. 349
  - Plan du bureau de la Justice ; une institution de cinq étages, schématisée à partir des pièces confirmées
  - Chap. 400, p. 194-199 et 206 — le bureau judiciaire / salle d'interrogatoire 1010, signalé sur la porte, avec bureau formel, sièges visiteurs et bibliothèques toute hauteur sur les murs du fond et latéraux.
  - Plan du bureau de la Justice ; paroi vitrée, visites enregistrées de dix minutes, tiroir de transfert
  - Les pièces sont dessinées, aucun passage entre elles ne l'est ; cette circulation est reconstruite.
- **Sources du mobilier et des détails :**
  - Plan du bureau de la Justice ; une institution de cinq étages, schématisée à partir des pièces confirmées
  - Chap. 400, p. 194-199 et 206 — le bureau utilisé pour l'interrogatoire judiciaire dans la pièce 1010 signalée.
  - Chap. 400, p. 194-199 et 206 — des bibliothèques toute hauteur, chargées de dossiers et volumes, bordent le bureau judiciaire derrière et à côté de la table.
  - Chap. 400, p. 194-199 et 206 — la pièce 1010 signalée contient un bureau face aux chaises visiteurs et des murs de bibliothèques toute hauteur.
- **Limite connue :** 1 liaison(s) ou volume(s) restent déduits pour assurer la continuité de la visite.

## Salle de projection — `tier-2-screening-room`

- **Pont :** tier-2
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 1
- **Emprises relevées :** 87.5 × 45.5 m
- **Hauteurs sous plafond :** 10 m
- **Éléments modélisés :** 6 seat, 5 platform, 2 painting, 2 pillar
- **Objets directement visibles dans les cases :** 15/15
- **Sources de la pièce :**
  - Chap. 359 — le théâtre : une scène à cadre sous son rideau, des loges le long des murs latéraux, et l'écran posé sur la scène.
- **Sources du mobilier et des détails :**
  - Chap. 359 — le théâtre : une scène à cadre sous son rideau, des loges le long des murs latéraux, et l'écran posé sur la scène.

## Zone de protection des témoins VIP — `tier-2-vip-witness-protection-area`

- **Pont :** tier-2, interior-justice-bureau
- **Preuve de l’enveloppe :** case du manga, plan publié
- **Sous-espaces :** 2
- **Emprises relevées :** 33.25 × 45.5 m ; 19.5 × 13.5 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 6 seat, 2 cabinet, 2 painting, 2 table, 1 bed
- **Objets directement visibles dans les cases :** 13/13
- **Sources de la pièce :**
  - Plan des ponts, chap. 349
  - Chap. 400, p. 200-204 — la suite de protection VIP : murs moulurés et plafonds à caissons, lustre et miroir ovale, avec lit, table à manger, fauteuils, guéridons, tableaux et meubles ; la subdivision exacte n'est pas publiée.
- **Sources du mobilier et des détails :**
  - Chap. 400, p. 200-204 — la suite de protection VIP montre des murs et plafonds richement moulurés avec lit, mobilier de repas, fauteuils, guéridons, meubles, tableaux et miroir ovale ; les positions sont ajustées dans le contour non publié de la suite.

## Palais de justice central — `tier-3-central-courthouse`

- **Pont :** tier-3, interior-courthouse
- **Preuve de l’enveloppe :** plan publié
- **Sous-espaces :** 2
- **Emprises relevées :** 35 × 59.5 m ; 24 × 18 m
- **Hauteurs sous plafond :** 8 m
- **Éléments modélisés :** 3 table, 2 counter, 1 platform
- **Objets directement visibles dans les cases :** 0/6
- **Espaces internes :** Salle d’audience
- **Sources de la pièce :**
  - Plan des ponts, chap. 349
  - Plan du tribunal ; le siège du magistrat, les greffiers, le parquet et la défense
- **Sources du mobilier et des détails :**
  - Plan du tribunal ; le siège du magistrat, les greffiers, le parquet et la défense

## Hôpital central — `tier-3-central-hospital`

- **Pont :** tier-3, interior-central-hospital
- **Preuve de l’enveloppe :** case du manga, plan publié, liaison déduite
- **Sous-espaces :** 6
- **Emprises relevées :** 52.5 × 52.5 m ; 25 × 15 m ; 15 × 7.25 m ; 15 × 6.5 m ; 41.5 × 6.5 m ; 1.5 × 16.5 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 4 bed, 2 cabinet, 1 counter, 1 table
- **Objets directement visibles dans les cases :** 0/8
- **Espaces internes :** Clinique médicale centrale ; Réserve médicale ; Zone du personnel médical ; Liaison avec l’institut de recherche ; Couloir du service
- **Sources de la pièce :**
  - Chap. 358 — la coupe annotée du quatrième jour, qui pointe cette pièce et dit donc où elle se trouve sur la longueur du navire. La reconstruction la mettait au milieu, en avant de ce que donne la page.
  - Plan de l'hôpital central ; le quartier médical compte trois cliniques
  - Plan de l'hôpital central ; médicaments et provisions de voyage
  - Plan de l'hôpital central ; la limite interne avec l'institut n'est pas publiée
  - Les pièces sont dessinées, aucun passage entre elles ne l'est ; cette circulation est reconstruite.
- **Sources du mobilier et des détails :**
  - Plan de l'hôpital central ; le quartier médical compte trois cliniques
- **Limite connue :** 1 liaison(s) ou volume(s) restent déduits pour assurer la continuité de la visite.

## Commissariat central — `tier-3-central-police-station`

- **Pont :** tier-3, interior-police-station
- **Preuve de l’enveloppe :** case du manga, plan publié
- **Sous-espaces :** 4
- **Emprises relevées :** 35 × 45.5 m ; 22 × 15 m ; 10 × 4 m ; 11 × 6 m
- **Hauteurs sous plafond :** 16, 6 m
- **Éléments modélisés :** 9 platform, 8 pillar, 2 counter, 2 painting, 1 basin
- **Objets directement visibles dans les cases :** 20/22
- **Espaces internes :** Atrium principal ; Entrée ; Section de sécurité publique
- **Sources de la pièce :**
  - Plan des ponts, chap. 349
  - La double page de l'atrium du poste de police : quatre niveaux de galerie le long de chaque mur latéral, la colonnade qui se dresse dessous, la fontaine à gradins au sol et la façade d'entrée au fond, ses portes sous une marquise entre deux enseignes suspendues, et le lanternon soulevé au milieu du plafond.
  - Plan du poste de police ; l'atrium et sa fontaine, le guichet de sécurité publique
- **Sources du mobilier et des détails :**
  - La double page de l'atrium du poste de police : quatre niveaux de galerie le long de chaque mur latéral, la colonnade qui se dresse dessous, la fontaine à gradins au sol et la façade d'entrée au fond, ses portes sous une marquise entre deux enseignes suspendues, et le lanternon soulevé au milieu du plafond.
  - Plan du poste de police ; l'atrium et sa fontaine, le guichet de sécurité publique

## Cinéplexe — `tier-3-cineplex`

- **Pont :** tier-3, interior-cineplex
- **Preuve de l’enveloppe :** case du manga, plan publié, reconstruction cartographique, liaison déduite
- **Sous-espaces :** 15
- **Emprises relevées :** 52.5 × 42 m ; 12.5 × 15 m ; 5 × 15 m ; 15 × 17.5 m ; 56 × 5 m ; 7 × 20 m
- **Hauteurs sous plafond :** 8, 6 m
- **Éléments modélisés :** 48 seat, 9 painting, 2 counter, 1 pillar, 1 platform
- **Objets directement visibles dans les cases :** 4/61
- **Espaces internes :** Confiserie ; Foyer (bâbord) ; Couloir des salles (salles 1 à 8) ; Foyer (tribord) ; Billetterie ; Hall des salles ; Salle 1 ; Salle 2 ; Salle 3 ; Salle 4 ; Salle 5 ; Salle 6 ; Salle 7 ; Salle 8
- **Sources de la pièce :**
  - Plan des ponts, chap. 349 — huit salles
  - Chap. 393 — le hall : le stand de restauration, les guichets et l'allée qui descend vers les salles.
  - Les pièces sont dessinées, aucun passage entre elles ne l'est ; cette circulation est reconstruite.
  - Reconstruit pour desservir les huit salles nommées depuis le hall ; aucun dessin ne le montre.
  - Le plan du cinéplexe sur /ship nomme huit salles ; leur ordre depuis le hall est celui de la reconstruction.
- **Sources du mobilier et des détails :**
  - Chap. 393 — le hall : le stand de restauration, les guichets et l'allée qui descend vers les salles.
  - Le plan du cinéplexe sur /ship accroche l'affiche du film au-dessus de l'allée qui descend vers les salles.
  - Disposition de sièges reconstruite et répétée dans les huit salles nommées ; aucune source publiée n'établit son attribution salle par salle.
  - Le plan du cinéplexe sur /ship nomme ses huit salles d'après l'écran qu'elles portent ; chaque écran se dresse sur le mur qui fait face à l'entrée.
- **Limite connue :** 3 liaison(s) ou volume(s) restent déduits pour assurer la continuité de la visite.

## Bureau de la famille Heil-Ly — `tier-3-heilly-family-office`

- **Pont :** tier-3, interior-heilly-office
- **Preuve de l’enveloppe :** plan publié
- **Sous-espaces :** 3
- **Emprises relevées :** 52.5 × 35 m ; 16 × 10 m ; 6 × 2 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 1 cabinet, 1 table
- **Objets directement visibles dans les cases :** 0/2
- **Espaces internes :** Locaux déclarés ; Entrée publique
- **Sources de la pièce :**
  - Plan des ponts, chap. 349
  - Plan du bureau Heil-Ly ; les locaux déclarés et l'entrée fouillée par les soldats kakinois
- **Sources du mobilier et des détails :**
  - Plan du bureau Heil-Ly ; les locaux déclarés et l'entrée fouillée par les soldats kakinois

## Pont d'observation — `tier-3-observation-deck`

- **Pont :** tier-3
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 1
- **Emprises relevées :** 17 × 129.5 m
- **Hauteurs sous plafond :** 9 m
- **Éléments modélisés :** 1 window
- **Objets directement visibles dans les cases :** 1/1
- **Sources de la pièce :**
  - Chap. 358 — la coupe annotée du quatrième jour, qui pointe cette pièce et dit donc où elle se trouve sur la longueur du navire. La reconstruction la mettait à l'arrière, sur un plan lu avec les axes croisés.
- **Sources du mobilier et des détails :**
  - Chap. 380 — la baie par laquelle le pont d’observation regarde dehors. La planche la dessine courbe ; elle court le long du mur extérieur, c’est-à-dire la face avant depuis que le chap. 358 met le pont à la proue. Ce qu’elle donne à voir est dessiné aussi : la ville de conteneurs des ponts inférieurs, puis la mer, puis l’horizon et les nuages au-dessus.

## Bloc résidentiel première classe — `tier-3-residential-first-class`

- **Pont :** tier-3-b, interior-tier3-cabins
- **Preuve de l’enveloppe :** plan publié, liaison déduite
- **Sous-espaces :** 16
- **Emprises relevées :** 63 × 52.5 m ; 45 × 2.5 m ; 15 × 10 m ; 4 × 3 m ; 10 × 2.5 m ; 2 × 11 m ; 8 × 3.67 m ; 1.8 × 1.8 m
- **Hauteurs sous plafond :** 3.5 m
- **Éléments modélisés :** 6 bed, 6 cabinet
- **Objets directement visibles dans les cases :** 0/12
- **Espaces internes :** Couloir ; Suite de première classe 1 ; WC — Suite de première classe 1 ; Suite de première classe 2 ; WC — Suite de première classe 2 ; Suite de première classe 3 ; WC — Suite de première classe 3 ; Escalier vers l’étage ; Coursive de l'étage ; Mono-cabine de l'étage 1 ; WC — Mono-cabine 1 ; Mono-cabine de l'étage 2 ; WC — Mono-cabine 2 ; Mono-cabine de l'étage 3 ; WC — Mono-cabine 3
- **Sources de la pièce :**
  - Plan des ponts, chap. 349
  - Plan de la première classe, chap. 393
  - Plan de la première classe, chap. 393 — le plan dessine une cellule WC contre le mur mitoyen de chaque cabine de première classe, et une dans l'angle de chaque mono-cabine de l'étage.
  - Les pièces sont dessinées, aucun passage entre elles ne l'est ; cette circulation est reconstruite. La cabine simple est à l’étage, et aucun escalier n’y monte sur le dessin.
- **Sources du mobilier et des détails :**
  - Plan de la première classe, chap. 393
- **Limite connue :** 1 liaison(s) ou volume(s) restent déduits pour assurer la continuité de la visite.

## Chambre 3101 (piège Heil-Ly) — `tier-3-residential-room-3101`

- **Pont :** tier-3-b, interior-room-3101
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 3
- **Emprises relevées :** 21 × 10.5 m ; 9 × 9 m ; 5 × 5 m
- **Hauteurs sous plafond :** 3.5 m
- **Éléments modélisés :** 3 cabinet, 2 basin, 1 bars, 1 bed
- **Objets directement visibles dans les cases :** 5/7
- **Espaces internes :** Séjour ; Salle de bains anormale
- **Sources de la pièce :**
  - Chap. 398, p. 150-153 — la chambre 3101 et son seuil de salle de bains piégé par téléportation
  - Chap. 393, p. 64-67 — la 3101 est montrée depuis ses deux extrémités : deux couchettes superposées occupent une moitié du mur du fond, une penderie ouverte avec vêtements suspendus, tiroirs et étagères occupe l'autre, et la porte de la salle de bains se trouve sur le mur latéral ; la profondeur exacte de la cloison est reconstruite.
  - Chap. 398, p. 150-153 — le seuil de la salle de bains constitue le piège Heil-Ly : le franchir transfère la victime dans la planque au lieu de mener simplement à la pièce d'eau ; les appareils et la cloison sont reconstruits d'après le plan de la cabine.
- **Sources du mobilier et des détails :**
  - Chap. 393, p. 64-67 — deux couchages superposés sont visibles contre le mur du fond de la chambre 3101.
  - Chap. 393, p. 64-67 — l'échelle se trouve sur le côté libre du lit superposé.
  - Chap. 393, p. 64-67 — la penderie ouverte se trouve à côté du lit superposé, sur le mur du fond.
  - Chap. 393, p. 64-67 — une rangée de tiroirs intégrés se trouve à côté de la penderie ouverte.
  - Chap. 393, p. 64-67 — des étagères ouvertes intégrées complètent le mur de rangement à côté des tiroirs.
  - Chap. 398, p. 150-153 — la pièce d'eau se trouve au-delà du seuil piégé ; la position exacte de cet appareil est reconstruite d'après le plan de cabine, et non visible à travers le portail actif.

## Bloc résidentiel standard — `tier-3-residential-standard`

- **Pont :** tier-3-c, interior-general-cabins
- **Preuve de l’enveloppe :** plan publié
- **Sous-espaces :** 14
- **Emprises relevées :** 63 × 70 m ; 36 × 8 m ; 6 × 6 m
- **Hauteurs sous plafond :** 3 m
- **Éléments modélisés :** 12 bed, 12 cabinet
- **Objets directement visibles dans les cases :** 0/24
- **Espaces internes :** Couloir de rassemblement ; Cabine standard 1 ; Cabine standard 2 ; Cabine standard 3 ; Cabine standard 4 ; Cabine standard 5 ; Cabine standard 6 ; Cabine standard 7 ; Cabine standard 8 ; Cabine standard 9 ; Cabine standard 10 ; Cabine standard 11 ; Cabine standard 12
- **Sources de la pièce :**
  - Plan des ponts, chap. 349
  - Plan des cabines standard ; le couloir sert aussi de point de rassemblement
- **Sources du mobilier et des détails :**
  - Plan des cabines standard ; le couloir sert aussi de point de rassemblement

## Dortoir du commissariat central — `tier-3-residential-units`

- **Pont :** tier-3
- **Preuve de l’enveloppe :** liaison déduite
- **Sous-espaces :** 1
- **Emprises relevées :** 35 × 14 m
- **Éléments modélisés :** aucun objet non attesté
- **Objets directement visibles dans les cases :** 0/0
- **Sources de la pièce :**
  - Le catalogue situe le dortoir dans le commissariat ; rien n'en dessine la place.
- **Limite connue :** 1 liaison(s) ou volume(s) restent déduits pour assurer la continuité de la visite.

## Passage central ponts 4–5 — `tier-4-central-passage`

- **Pont :** tier-4
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 1
- **Emprises relevées :** 173.25 × 14 m
- **Éléments modélisés :** aucun objet non attesté
- **Objets directement visibles dans les cases :** 0/0
- **Sources de la pièce :**
  - Chap. 380 — Shizuku est située dans ce passage
- **Limite connue :** aucune case ne justifie davantage de mobilier dans ce volume.

## Bureau de la famille Ei-I — `tier-4-ei-i-family-office`

- **Pont :** tier-4-b
- **Preuve de l’enveloppe :** plan publié
- **Sous-espaces :** 1
- **Emprises relevées :** 61.2 × 21 m
- **Éléments modélisés :** aucun objet non attesté
- **Objets directement visibles dans les cases :** 0/0
- **Sources de la pièce :**
  - Coupe du vaisseau, chap. 380 — la planche pose la pièce et n’en donne pas le dedans.
- **Limite connue :** aucune case ne justifie davantage de mobilier dans ce volume.

## Installations de recyclage et d’assainissement — `tier-4-recycling-sewage-facilities`

- **Pont :** tier-4, tier-5
- **Preuve de l’enveloppe :** plan publié
- **Sous-espaces :** 2
- **Emprises relevées :** 255.5 × 15.75 m ; 210 × 17.5 m
- **Hauteurs sous plafond :** 8 m
- **Éléments modélisés :** aucun objet non attesté
- **Objets directement visibles dans les cases :** 0/0
- **Espaces internes :** Niveau technique et recyclage
- **Sources de la pièce :**
  - Plan des ponts, chap. 349
  - Plan des ponts, chap. 349 — installations techniques vers le pont 4
- **Limite connue :** aucune case ne justifie davantage de mobilier dans ce volume.

## Salle de conférence de l'armée royale de Kakin — `tier-4-royal-army-conference-room`

- **Pont :** tier-4, interior-royal-army-conference
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 2
- **Emprises relevées :** 52.5 × 52.5 m ; 20 × 15 m
- **Hauteurs sous plafond :** 5 m
- **Éléments modélisés :** 15 seat, 3 table, 2 painting, 1 counter
- **Objets directement visibles dans les cases :** 21/21
- **Espaces internes :** Salle de conférence
- **Sources de la pièce :**
  - Chap. 350 — réunion de coordination de la sécurité
  - Chap. 380, p. 189 — la salle de briefing de l'armée royale est montrée depuis le public : un pupitre se dresse devant un écran de projection vierge et un tableau de cartes, face à une table en U entourée de quinze sièges.
- **Sources du mobilier et des détails :**
  - Chap. 380, p. 189 — le grand écran de projection vierge est suspendu derrière l'orateur.
  - Chap. 380, p. 189 — un tableau de cartes clair, plus petit, est suspendu à gauche de l'écran de projection.
  - Chap. 380, p. 189 — l'orateur se tient à côté du pupitre, devant l'écran de projection.
  - Chap. 380, p. 189 — trois éléments de table joints forment le U au centre de la salle de briefing.
  - Chap. 380, p. 189 — quinze sièges entourent la table en U pendant le briefing de l'armée royale.

## Bureau de la famille Xi-Yu — `tier-4-xi-yu-family-office`

- **Pont :** tier-4, interior-xi-yu-office
- **Preuve de l’enveloppe :** plan publié
- **Sous-espaces :** 3
- **Emprises relevées :** 56 × 21 m ; 16.1 × 8.1 m ; 6 × 2.1 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 1 table
- **Objets directement visibles dans les cases :** 0/1
- **Espaces internes :** Bureau de réunion de la famille ; Entrée gardée
- **Sources de la pièce :**
  - Plan des ponts, chap. 349
  - Plan du bureau Xi-Yu ; la section avant, dans les « dents » du Black Whale
- **Sources du mobilier et des détails :**
  - Plan du bureau Xi-Yu ; la section avant, dans les « dents » du Black Whale

## Zone 37564 — `tier-5-area-37564`

- **Pont :** tier-5-b
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 1
- **Emprises relevées :** 28 × 14 m
- **Éléments modélisés :** aucun objet non attesté
- **Objets directement visibles dans les cases :** 0/0
- **Sources de la pièce :**
  - Chap. 358 — point de rassemblement 37564
- **Limite connue :** aucune case ne justifie davantage de mobilier dans ce volume.

## Réfectoire central — `tier-5-central-dining-hall`

- **Pont :** tier-5
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 1
- **Emprises relevées :** 31.5 × 31.5 m
- **Hauteurs sous plafond :** 9 m
- **Éléments modélisés :** 6 table, 2 duct, 1 counter, 1 painting, 1 platform
- **Objets directement visibles dans les cases :** 11/11
- **Sources de la pièce :**
  - Chap. 358 — la taxe Buor sur les repas. Le chap. 371 dessine la salle elle-même, et dit ce qu'elle est : le seul réfectoire du vaisseau, plein d'un bout à l'autre à l'heure du repas, la table des trois familles mafieuses au milieu de la foule, et au-dessus une charpente de fermes, de passerelles et de gaines bien trop haute pour les 4,5 m du pont — la salle reçoit donc 9 m, au même titre que la mezzanine du casino lui vaut ses 8.
- **Sources du mobilier et des détails :**
  - Chap. 377 — la salle à l'heure du repas : de longues planches basses en rangées, les convives assis à même le pont le long d'elles — pas un banc dans la salle — et au milieu l'estrade que prend la Brigade, assez basse pour qu'on s'y assoie de même.
  - Chap. 377 — le mur du fond du réfectoire : un comptoir de service continu tenu par le personnel, devant une rangée de clients assis, où passent plats et boissons.
  - Chap. 377 — six grands panneaux de menu encadrés sont suspendus au-dessus du comptoir, leurs colonnes de plats et de prix visibles depuis la salle.
  - Chap. 371 — la salle dessinée pleine, sous la charpente qui lui donne sa hauteur : fermes et passerelles au-dessus, et les gaines qui courent entre elles, hors d'atteinte de toutes les têtes de la salle.

## Bureau de la famille Cha-R — `tier-5-cha-r-family-office`

- **Pont :** tier-5, interior-cha-r-office
- **Preuve de l’enveloppe :** case du manga, plan publié, liaison déduite
- **Sous-espaces :** 5
- **Emprises relevées :** 42 × 31.5 m ; 18 × 15 m ; 18 × 7 m ; 21 × 23.5 m ; 1.5 × 23.5 m
- **Hauteurs sous plafond :** 4 m
- **Éléments modélisés :** 4 cabinet, 2 bed, 2 table, 1 counter, 1 painting
- **Objets directement visibles dans les cases :** 9/10
- **Espaces internes :** Salle à l’emblème ; Salle des couchettes ; Salle de surveillance ; Couloir du bureau
- **Sources de la pièce :**
  - Plan des ponts, chap. 349
  - Chap. 379-380 — la salle principale Cha-R porte l'emblème de la famille et trois portes différentes : une porte métallique à motif de coffre, une porte métallique à face de robot et une porte en bois.
  - Chap. 379, p. 16 — le compartiment attenant à la salle de l'emblème contient des lits superposés et des bibliothèques ; un homme est allongé sur la couchette inférieure sous Feitan.
  - Chap. 380 — la salle de surveillance séparée s'ouvre par la porte blanche depuis la salle de l'emblème et affiche les images des zones clés du pont 5.
  - Les pièces sont dessinées, aucun passage entre elles ne l'est ; cette circulation est reconstruite. L’entrée est la porte blindée à serrure électronique qui y est dessinée, surveillée par deux caméras.
- **Sources du mobilier et des détails :**
  - Plan du bureau Cha-R ; la section centrale, près de l’hélice
  - Chap. 379-380 — la première porte métallique possède un panneau central en relief évoquant un coffre.
  - Chap. 379, p. 16 — des lits superposés bordent le compartiment attenant ; la couchette inférieure est occupée sous Feitan.
  - Chap. 380 — le mur sombre d'écrans de vidéosurveillance montre les enregistrements de la planque et des zones clés du pont 5.
  - Chap. 380 — l'opérateur de vidéosurveillance travaille à la table face au mur d'écrans.
  - Chap. 379-380 — l'emblème Cha-R est peint en panneau graphique sombre dans la salle principale.
  - Chap. 379-380 — la deuxième porte métallique porte quatre marques sombres disposées comme les yeux et les pieds d'un robot et dissimule le passage secret.
  - Chap. 379-380 — une porte en bois ordinaire complète les trois portes distinctes de la salle à l'emblème.
  - Chap. 379, p. 16 — plusieurs lits superposés sont visibles le long du mur du compartiment.
  - Chap. 379, p. 16 — de hautes bibliothèques partagent le compartiment attenant avec les lits superposés.
- **Limite connue :** 1 liaison(s) ou volume(s) restent déduits pour assurer la continuité de la visite.

## Entrée du hangar — `tier-5-hangar-entrance`

- **Pont :** tier-5
- **Preuve de l’enveloppe :** case du manga
- **Sous-espaces :** 1
- **Emprises relevées :** 28 × 6.5 m
- **Éléments modélisés :** aucun objet non attesté
- **Objets directement visibles dans les cases :** 0/0
- **Sources de la pièce :**
  - Chap. 358 — la coupe annotée du quatrième jour, qui pointe cette pièce et dit donc où elle se trouve sur la longueur du navire. La reconstruction la mettait dans le coin arrière tribord.
- **Limite connue :** aucune case ne justifie davantage de mobilier dans ce volume.

## Clinique du pont 5 — `tier-5-medical-clinic`

- **Pont :** tier-5
- **Preuve de l’enveloppe :** plan publié
- **Sous-espaces :** 1
- **Emprises relevées :** 24.5 × 31.5 m
- **Éléments modélisés :** aucun objet non attesté
- **Objets directement visibles dans les cases :** 0/0
- **Sources de la pièce :**
  - Plan des ponts, chap. 349
- **Limite connue :** aucune case ne justifie davantage de mobilier dans ce volume.

## Cabines standard du pont 5 (avant) — `tier-5-standard-cabins`

- **Pont :** tier-5-b, interior-tier5-standard-cabins
- **Preuve de l’enveloppe :** plan publié
- **Sous-espaces :** 15
- **Emprises relevées :** 63 × 57.75 m ; 63 × 15.75 m ; 36 × 8 m ; 6 × 6 m
- **Hauteurs sous plafond :** 3 m
- **Éléments modélisés :** 12 bed, 12 cabinet
- **Objets directement visibles dans les cases :** 0/24
- **Espaces internes :** Cabines standard du pont 5 (arrière) ; Couloir de rassemblement (avant) ; Cabine avant 1 ; Cabine avant 2 ; Cabine avant 3 ; Cabine avant 4 ; Cabine avant 5 ; Cabine avant 6 ; Cabine avant 7 ; Cabine avant 8 ; Cabine avant 9 ; Cabine avant 10 ; Cabine avant 11 ; Cabine avant 12
- **Sources de la pièce :**
  - Plan des ponts, chap. 349
  - Plan des cabines standard ; le couloir sert aussi de point de rassemblement
- **Sources du mobilier et des détails :**
  - Plan des cabines standard ; le couloir sert aussi de point de rassemblement

## Entrepôt sous contrôle Cha-R — `tier-5-warehouse`

- **Pont :** tier-5
- **Preuve de l’enveloppe :** plan publié
- **Sous-espaces :** 1
- **Emprises relevées :** 112 × 49 m
- **Hauteurs sous plafond :** 12 m
- **Éléments modélisés :** 32 cabinet, 4 counter, 1 camera
- **Objets directement visibles dans les cases :** 0/37
- **Sources de la pièce :**
  - Plan de l'entrepôt ; la cale sous contrôle Cha-R, ses rangées de caisses et son unique entrée gardée
- **Sources du mobilier et des détails :**
  - Plan de l'entrepôt ; la cale sous contrôle Cha-R, ses rangées de caisses et son unique entrée gardée
  - Plan de l'entrepôt ; la cale sous contrôle Cha-R, ses rangées de caisses et son unique entrée gardée ; le plan précise que la caméra a été posée après le chargement
