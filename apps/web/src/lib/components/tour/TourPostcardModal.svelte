...
>
  ...
  >
    ...
    <div class="relative flex justify-center items-center w-full min-h-[40vh]">
      ...
      >
        <!-- The photo -->
        <img src={photoUrl} alt="Screenshot" class="w-full h-full object-cover border border-gray-300" style="min-height: 0;" />

        <!-- Overlay Text -->
        <div class="absolute bottom-4 left-0 w-full text-center px-4 font-serif italic text-3xl text-zinc-800 drop-shadow-sm tracking-wide">
          {text}
        </div>

        <!-- Stamps -->
        {#if stampType === 'kakin'}
          <div class="absolute -top-6 -right-6 w-32 h-32 rounded-full border-[6px] border-red-700/80 text-red-700/80 flex items-center justify-center rotate-[15deg] mix-blend-multiply pointer-events-none opacity-90">
            <div class="text-center font-bold uppercase tracking-tighter leading-tight" style="font-size: 1.1rem; border-top: 2px dashed rgba(185,28,28,0.8); border-bottom: 2px dashed rgba(185,28,28,0.8); padding: 4px 0;">
              KAKIN EMPIRE<br/>
              <span class="text-sm">Official VIP</span>
            </div>
          </div>
        {:else}
          <div class="absolute -top-4 -right-4 w-28 h-28 border-[4px] border-blue-800/80 text-blue-800/80 flex items-center justify-center rotate-[-10deg] mix-blend-multiply pointer-events-none opacity-90">
            <div class="text-center font-bold uppercase tracking-widest leading-none" style="font-size: 1.5rem;">
              HUNTER<br/>
        ...
        <div class="absolute inset-0 pointer-events-none" style="background: linear-gradient(135deg, rgba(255,255,255,0.1) 0%, rgba(0,0,0,0.05) 100%);"></div>
      </div>
    </div>

    <!-- Controls -->
    <div class="bg-zinc-900 border border-zinc-800 rounded-lg p-6 flex flex-col md:flex-row gap-6 shadow-xl w-full mx-auto justify-between items-center text-white">
