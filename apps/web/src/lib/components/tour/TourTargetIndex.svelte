  ...
  {:else if props.mode === 'solid'}
    <ul class="max-h-56 overflow-y-auto rounded border border-[#333]">
      {#each props.solidGroups as group (group.tier.id)}
        ...
        >
          {props.nameOf(group.tier)}
        </li>
        {#each group.solids as solid (solid.id)}
          <li>
            <button
              type="button"
              onclick={() => props.onSolid(solid.spaceId, solid.id)}
              ...
              style:background={props.isSolidActive(solid.id)
                ? `color-mix(in srgb, ${props.techniqueColor} 18%, transparent)`
                : undefined}
            >
              <span class="truncate">{props.nameOf(solid)}</span>
              <span class="flex shrink-0 items-baseline gap-1.5"
                ><span class="truncate text-[9px] text-[#FFFFF0]/40">{props.roomName(solid)}</span
                ><span
                  class="shrink-0 rounded border px-1 py-px text-[9px] uppercase {props.provenanceClass(
                    solid,
                  )}">{props.provenanceLabel(solid)}</span
                ></span
              >
            </button>
          </li>
          ...
          {#each group.spaces as space (space.id)}
            <li>
              <button
                ...
                class="flex w-full items-center justify-between gap-2 px-2.5 py-1.5 text-left text-xs text-[#FFFFF0]/80 transition-colors hover:text-[#FFFFF0]"
                style:background={activeSpaceStyle(space.id)}
              >
                <span class="truncate">{props.nameOf(space)}</span><span
                  class="shrink-0 rounded border px-1 py-px text-[9px] uppercase {props.provenanceClass(
                    space,
                  )}">{props.provenanceLabel(space)}</span
                >
              </button>
            </li>
          {/each}
