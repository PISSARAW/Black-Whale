<script lang="ts">
  ...
  import {
    ...
    currentCampaignScenario,
  } from '$lib/strategy/campaign/engine'
  import {
          ...
          createStrategySave(
            {
              ...
              selectedFactionId: playerFactionId,
              turns: structuredClone(simStore.turnHistory).map((turn, index) => ({
                ...turn,
                turn: index + 1,
              })),
            },
...
<main class="strategy-shell min-h-[calc(100vh-4rem)] bg-[#020617] text-sky-50 font-sans">
  ...
  {:else if !ready}
    ...
    >
      <div
        class="mb-4 flex items-center gap-3 rounded-full border border-sky-500/30 bg-sky-950/40 px-5 py-2 shadow-[0_0_15px_rgba(14,165,233,0.2)]"
      >
    ...
    >
      {#each data.scenarios as operation, index (operation.id)}
        ...
        >
          ...
          >
          <small
            class="text-xs leading-relaxed text-sky-200/50 group-aria-[current=page]:text-amber-100/70"
  ...
  {:else}
    <div
      class="grid min-h-[calc(100vh-4rem)] grid-cols-1 md:grid-cols-[minmax(22rem,28rem)_minmax(0,1fr)]"
    >
      <TourModeFullscreen />
      <aside class="flex flex-col border-r border-sky-900/40 bg-[#0a0f1c]/95 backdrop-blur-md">
        ...
        <div
          class="flex-1 overflow-y-auto p-5 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-sky-900/50"
        >
          {#if simStore.scenarioEvent}
            ...
            >
              ...
              >
              <p class="mt-2 text-xs leading-relaxed text-amber-100/70">
                {simStore.scenarioEvent.description}
              </p>
            </section>
          {/if}
          <section class="mb-8">
            <div
              class="rounded-xl border border-sky-900/40 bg-gradient-to-br from-[#101827] to-[#0a0f1c] p-5 shadow-[0_0_20px_rgba(14,165,233,0.1)] {objective?.complete
                ? 'border-emerald-500/50 shadow-[0_0_20px_rgba(52,211,153,0.15)]'
                : ''}"
            >
              ...
              <div
                class="mt-2 flex items-center justify-between text-[10px] font-bold uppercase tracking-widest text-sky-200/50"
              >
                <span
            ...
            {#if pendingOrders.length}
              <ul class="space-y-3">
                {#each pendingOrders as order (order.characterId)}
                  <li
                    class="flex items-center justify-between gap-3 rounded-lg border border-sky-900/40 bg-[#060b14]/50 p-3 shadow-inner"
                  >
                    <div class="min-w-0">
                      <strong class="block truncate text-xs text-sky-100"
                        >{memberName(order.characterId)}</strong
                      >
          ...
          <section class="border-t border-sky-900/30 pt-6">
            ...
            <div
              class="max-h-40 overflow-y-auto rounded-lg border border-sky-900/40 bg-[#040810] p-4 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-sky-900/50"
              aria-live="polite"
            >
              {#each simStore.turnReports as report, index (`${index}:${report}`)}<p
                  class="mb-2 text-[10px] leading-relaxed text-sky-200/60 last:mb-0"
                >
                  <span class="mr-1 text-sky-500/50">›</span>
                  {report}
                </p>{/each}
            </div>
          </section>
        ...
        <footer class="border-t border-sky-900/50 bg-[#060b14]/80 p-5">
          {#if errorMessage}<p
              class="mb-3 rounded bg-red-950/40 p-2 text-xs font-medium text-red-400"
              role="alert"
            >
              {errorMessage}
            </p>{/if}
      ...
      <section
        class="flex min-w-0 flex-col bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-sky-900/20 via-[#020617] to-black p-4 sm:p-8"
      >
        {#if simStore.gameOver}
          <StrategyDebrief
            ...
            oncontinue={campaignSave &&
            currentCampaignScenario(campaignSave.campaign)?.id === data.scenario?.id
              ? continueCampaign
              : undefined}
          />
        {/if}
        <div
          class="mb-6 flex flex-col items-start gap-4 sm:flex-row sm:items-end sm:justify-between"
        >
          <div>
