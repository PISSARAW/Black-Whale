...
import { completeCampaignRun, initialCampaign, loadCampaign, saveCampaign, type CampaignStorage } from './campaign'
...
describe('persistent Hunt campaign', () => {
  it('records attempts, mastery, best wins and consequences', () => {
    const campaign = completeCampaignRun(initialCampaign(), {
      contractId: 'royal-apartments', hatsu: 'bungee-gum', outcome: 'reached', seconds: 120, wounds: ['left-leg'],
    })
    expect(campaign.contracts['royal-apartments']).toEqual({ attempts: 1, wins: 1, bestSeconds: 120 })
